...
fn bench_crt_filter(c: &mut Criterion) {
    ...
    let distortion_table: Vec<(u32, u32)> = (0..SCREEN_W * SCREEN_H).map(|i| {
        let x = i % SCREEN_W;
        let y = i / SCREEN_W;
    ...
    c.bench_function("crt_filter", |b| {
        b.iter(|| {
            crt_filter(black_box(&input), black_box(&mut output),
                      black_box(&sv_table), black_box(&distortion_table),
                      black_box(&crt_cfg), black_box(&mask_table), 0, 0);
        })
    });
}

fn bench_phosphor_bloom(c: &mut Criterion) {
    let mut buffer: Vec<u32> = (0..SCREEN_W * SCREEN_H).map(|i| {
...
criterion_group!(benches, bench_crt_filter, bench_phosphor_bloom, bench_scanline_glow);
