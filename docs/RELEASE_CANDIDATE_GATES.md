...
- The diagnostic observability runner passes and writes `observability-run.json`, `observability-run.md`, and focused `replay-runs/<scenario>/` evidence:
...
- `python scripts/run_diagnostic_observability.py --suite-dir target/diagnostics/scenario-suite` records the command status, git commit, verifier summary, first AI next action, focused replay status, expected-vs-actual replay exit/health/focus checks, and artifact pointers for debugging handoff.
