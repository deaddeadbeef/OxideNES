...
- `telemetry.json` includes `cpu_load_store_matrix` and the `cpu.load_store_matrix.results` probe, proving LDA/LDX/LDY indexed loads, including LDA indexed-indirect and indirect-indexed addressing, STA side effects through absolute,Y, absolute,X, zero-page,X, indexed-indirect, and indirect-indexed addressing, STX/STY indexed zero-page stores, and TAX/TAY/TXA/TYA flag outcomes with expected load mask `0x1F`, store mask `0x7F`, and sixteen matrix cases.
