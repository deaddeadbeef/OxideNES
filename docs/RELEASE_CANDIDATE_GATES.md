...
- `telemetry.json` includes `ppu_vblank_timing` and the `ppu.vblank_timing.nmi_window` probe, proving the cartridge-observed first/inter-NMI cadence stays inside the accepted NTSC timing window.
- Diagnostic scenario suites include the pass baseline, a non-default input-mask pass fixture, intentional joypad, CPU, PPU, mapper, DMA, APU, and timeout fixtures, including sprite-zero-hit and sprite-overflow PPU collision/evaluation fixtures, with expected-vs-actual debug-focus contracts, per-clause contract-match breakdowns, root baseline-comparison summaries, single-scenario replay args, a suite-level attention queue, plus `scenario-suite.json`, `scenario-suite.md`, `scenario-suite-observer.json`, and `scenario-suite-observer.md` indexes for automated regression triage.
