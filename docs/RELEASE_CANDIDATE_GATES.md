...
- `telemetry.json` includes `mapper3_rendered_chr_bank` and the `mapper3.rendered_chr_bank_switch` probe, proving CNROM CHR bank writes remain visible in host-sampled rendered pixels while background rendering is enabled.
- `telemetry.json` includes `mapper4_mmc3_a12_gate` and the `mapper4.mmc3_a12_irq_gate` probe, proving low-pattern background rendering suppresses MMC3 IRQ clocking while high-pattern background rendering still triggers the configured IRQ.
- `telemetry.json` includes `mapper4_mmc3_sprite_a12_gate` and the `mapper4.mmc3_sprite_a12_irq_gate` probe, proving low-pattern sprite rendering suppresses MMC3 IRQ clocking while high sprite pattern-table rendering still triggers the configured IRQ.
- `telemetry.json` includes `cpu_load_store_matrix` and the `cpu.load_store_matrix.results` probe, proving LDA/LDX/LDY indexed loads, including LDA indexed-indirect and indirect-indexed addressing plus LDX zero-page, LDX zero-page,Y, LDY zero-page,X, and absolute indexed addressing, STA side effects through absolute,Y, absolute,X, zero-page,X, indexed-indirect, and indirect-indexed addressing, STX/STY indexed zero-page stores, and TAX/TAY/TXA/TYA flag outcomes with expected load mask `0xFF`, store mask `0x7F`, and nineteen matrix cases.
- `telemetry.json` includes `cpu_alu_index_matrix` and the `cpu.alu_index_matrix.results` probe, proving AND immediate and zero-page, ORA/EOR immediate, INX/INY/DEX/DEY wraparound, and zero/negative flag outcomes with expected logic mask `0x0F`, index mask `0x0F`, and eight matrix cases.
- Diagnostic scenario suites include the pass baseline, generated input-mask pass fixtures for alternating, all-released, all-pressed, joypad-1-only pressed, joypad-2-only pressed, sparse-bit, and nibble-split controller masks, intentional joypad, CPU, PPU, mapper, DMA, APU, and timeout fixtures, including CPU status/BIT flag coverage plus sprite-zero-hit, sprite-overflow, scroll-seam, pixel-phase, and attribute-quadrant PPU fixtures with direct, hardware-bug false-positive, and hardware-bug false-negative sprite-overflow subcases, with expected-vs-actual debug-focus contracts, per-clause contract-match breakdowns, root baseline-comparison summaries, single-scenario replay args, a suite-level attention queue, plus `scenario-suite.json`, `scenario-suite.md`, `scenario-suite-observer.json`, and `scenario-suite-observer.md` indexes for automated regression triage.
- Host input-remapping regression tests prove custom keyboard bindings, custom controller face-button bindings, turbo gates, merged sources, and joypad mask serialization use the same library path as the game loop before release evidence narrows the `input_port_matrix` gap.
...
- `telemetry.json` includes PPU background pixel-phase frame-pixel samples so automated debuggers can localize alternating, low-plane, and high-plane tile phase regressions to `ppu.pixel_phase`.
- `telemetry.json` includes PPU attribute-quadrant frame-pixel samples and the expected attribute byte so automated debuggers can localize background palette quadrant regressions to `ppu.attribute_quadrant`.
