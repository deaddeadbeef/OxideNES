...
- The diagnostic observability runner passes and writes `observability-run.json` plus `observability-run.md`:
...
- `python scripts/run_diagnostic_observability.py --suite-dir target/diagnostics/scenario-suite` records the command status, git commit, verifier summary, first AI next action, and artifact pointers for debugging handoff.
