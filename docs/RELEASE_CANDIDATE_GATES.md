...
- `telemetry.json` includes `cpu_alu_index_matrix` and the `cpu.alu_index_matrix.results` probe, proving AND/ORA/EOR immediate and zero-page, INX/INY/DEX/DEY wraparound, and zero/negative flag outcomes with expected logic mask `0x3F`, index mask `0x0F`, and ten matrix cases.
