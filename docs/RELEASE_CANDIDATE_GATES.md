...
- The diagnostic observability runner passes and writes `observability-run.json`, `observability-run.md`, `diagnostic-debug-index.jsonl`, `diagnostic-debug-index.md`, `diagnostic-observability-analysis.json`, `diagnostic-observability-analysis.md`, `diagnostic-code-map.json`, `diagnostic-code-map.md`, `diagnostic-investigation-plan.json`, `diagnostic-investigation-plan.md`, and focused `replay-runs/<scenario>/` evidence:
...
- The diagnostic route evidence verifier passes and contract-checks both the all-route replay matrix and top-route full check:
...
- `python scripts/verify_diagnostic_observability.py --suite-dir target/diagnostics/scenario-suite` validates `observability-run.json`, root artifact pointers, debug-index rows, ranked hypotheses, code-map source/test paths, investigation-plan routes, optional comparison summaries, and focused replay evidence before AI-facing observability evidence is accepted.
...
- `python scripts/verify_diagnostic_route.py --suite-dir target/diagnostics/scenario-suite --require-matrix --require-top-route --expect-matrix-tests-skipped` validates route matrix schema/counts, per-route replay bundles, top-route expected-vs-actual health/focus checks, and narrow-test command results before route evidence is accepted.
