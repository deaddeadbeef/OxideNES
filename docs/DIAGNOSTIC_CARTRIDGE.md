...
summary, top-route, scenario, focus-domain, probe, and coverage queries. Start
with the e2e report to decide whether the diagnostic corpus is trusted, then
use the AI index or query CLI to choose a scenario card, failed probe, or
focus-domain route before opening full telemetry or replay bundles.
...
focus domain, failed probe id, top route, and coverage posture.
With
