...
`cpu_rmw_matrix_fault` corrupts the ASL zero-page source byte before the
generated cartridge exercises read-modify-write opcodes, proving RMW memory
write-back regressions localize to `cpu.rmw.asl`.
`cpu_rmw_addressing_matrix_fault` corrupts the ASL absolute source byte before
...
`cpu_load_store_matrix_fault` corrupts the load/store/transfer store mask
before the generated cartridge summarizes LDA/LDX/LDY loads, STA/STX/STY
...
before the generated cartridge summarizes AND/ORA/EOR immediate and zero-page
cases plus INX/INY/DEX/DEY wraparound cases, proving logical result flags and
index register regressions localize to
...
`cpu_status_matrix_fault` corrupts the CPU status/BIT matrix bit mask before
the generated cartridge summarizes BIT zero/negative/overflow flags and
explicit status set/clear opcodes, proving status flag regressions localize to
...
build/test/clippy status, offline cargo-audit status from the cached RustSec
database, and binary smoke evidence.
...
- Zero-page read-modify-write memory write-back for `ASL`, `ROL`, `LSR`,
  `ROR`, `INC`, and `DEC`
- Absolute and absolute,X read-modify-write memory write-back for `ASL`,
  `ROL`, `LSR`, `ROR`, `INC`, and `DEC`
...
Schema version `59` adds the `cpu_read_modify_write_matrix` cartridge test,
top-level `cpu_rmw_matrix` telemetry, and the `cpu.rmw_matrix.results` probe.
The cartridge records zero-page `ASL`, `ROL`, `LSR`, `ROR`, `INC`, and `DEC`
write-back sentinels and localizes the intentional `cpu_rmw_matrix_fault`
fixture to `cpu.rmw.asl`.

Schema version `60` adds the `cpu_rmw_addressing_matrix` cartridge test,
top-level `cpu_rmw_addressing_matrix` telemetry, and the
`cpu.rmw_addressing_matrix.results` probe. The cartridge records absolute
`ASL`, `LSR`, and `INC` write-back sentinels plus absolute,X `ROL`, `ROR`, and
...
Schema version `66` adds the `cpu_load_store_transfer_matrix` cartridge test,
...
The cartridge records AND/ORA/EOR immediate logical result and zero/negative
...
Schema version `70` adds the `cpu_status_bit_matrix` cartridge test, top-level
`cpu_status_matrix` telemetry, and the `cpu.status_matrix.results` probe. The
...
Schema version `76` extends the `cpu_load_store_transfer_matrix` generated
cartridge test with an indexed absolute `STA abs,X` store case. The
...
Schema version `78` extends the `cpu_load_store_transfer_matrix` generated
cartridge test with an indexed-indirect `STA (zp,X)` store case. The
load/store telemetry now expects six store cases, store mask `0x3F`, and
fourteen total load/store/transfer cases, continuing to narrow official store
addressing-mode coverage without changing the JSON shape.

Schema version `79` extends the `cpu_load_store_transfer_matrix` generated
cartridge test with an indirect-indexed `STA (zp),Y` store case. The
load/store telemetry now expects seven store cases, store mask `0x7F`, and
...
Schema version `81` extends the `cpu_load_store_transfer_matrix` generated
cartridge test with an indexed zero-page `LDX zp,Y` load case. The load/store
telemetry now expects six load cases, load mask `0x3F`, a retained
`load_x_zp_y_result` byte, and seventeen total load/store/transfer cases,
continuing to narrow official load addressing-mode coverage with an explicit
expected-vs-observed value for AI triage.

Schema version `82` extends the `cpu_load_store_transfer_matrix` generated
cartridge test with an indexed zero-page `LDY zp,X` load case. The load/store
telemetry now expects seven load cases, load mask `0x7F`, a retained
`load_y_zp_x_result` byte, and eighteen total load/store/transfer cases,
continuing to narrow official load addressing-mode coverage with an explicit
expected-vs-observed value for AI triage.
...
Schema version `85` extends the `cpu_alu_index_matrix` generated cartridge test
with zero-page `ORA zp` and `EOR zp` logical cases. The ALU/index telemetry now
expects six logic cases, logic mask `0x3F`, retained `ora_zp_result` and
`eor_zp_result` bytes, and ten total ALU/index cases, completing explicit
expected-vs-observed zero-page coverage for the official AND/ORA/EOR logical
accumulator family.
...
Scenario suite schema version `10` adds the `cpu_rmw_matrix_fault` negative
fixture. The fault corrupts the ASL zero-page source byte immediately before
test 37 executes, so the suite can localize read-modify-write write-back
regressions to `cpu.rmw.asl` with a paired AI route and replay command.

Scenario suite schema version `11` adds the `cpu_rmw_addressing_matrix_fault`
negative fixture. The fault corrupts the ASL absolute source byte immediately
...
Scenario suite schema version `17` adds the `cpu_load_store_matrix_fault`
negative fixture. The fault corrupts the load/store/transfer store mask
immediately before test 44 executes, so the suite can localize LDA/LDX/LDY,
STA/STX/STY, and TAX/TAY/TXA/TYA regressions to
`cpu.load_store.transfer_matrix` with a paired AI route and replay command.
...
immediate and zero-page result/flag regressions, and INX/INY/DEX/DEY
index-register wraparound regressions to
...
Scenario suite schema version `21` adds the `cpu_status_matrix_fault` negative
fixture. The fault corrupts the CPU status/BIT matrix bit mask immediately
