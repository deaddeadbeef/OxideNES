...
EXPECTED_SCENARIO_SUITE_SCHEMA = 16
EXPECTED_OBSERVER_SCHEMA = 2
EXPECTED_TELEMETRY_SCHEMA = 65
...
class SuiteVerifier:
    ...
    def verify_observer_actions(self, actions: list[Any]) -> None:
        ...
        if not isinstance(ppu_increment_32, dict):
            self.errors.append("missing observer action for ppu_vram_increment_32_fault")
            return

        self.expect_equal(
            ppu_increment_32.get("priority"),
            "known_divergence",
            "PPU increment-32 observer action priority",
        )
        self.expect_equal(
            ppu_increment_32.get("action_type"),
            "inspect_known_divergence",
            "PPU increment-32 observer action type",
        )
        self.expect_equal(
            ppu_increment_32.get("primary_artifact"),
            "ppu_vram_increment_32_fault/comparison.json",
            "PPU increment-32 observer primary_artifact",
        )
        ppu_increment_32_evidence = self.expect_list(
            ppu_increment_32.get("evidence"), "PPU increment-32 observer evidence"
        )
        self.expect_in(
            "focus_domain=ppu.registers.ppudata_increment_32",
        ...
        addressing = by_scenario.get("cpu_addressing_matrix_fault")
        if not isinstance(addressing, dict):
            self.errors.append("missing observer action for cpu_addressing_matrix_fault")
            return

        self.expect_equal(
            addressing.get("priority"),
            "known_divergence",
            "CPU addressing matrix observer action priority",
        )
        self.expect_equal(
            addressing.get("action_type"),
            "inspect_known_divergence",
            "CPU addressing matrix observer action type",
        )
        self.expect_equal(
            addressing.get("primary_artifact"),
            "cpu_addressing_matrix_fault/comparison.json",
            "CPU addressing matrix observer primary_artifact",
        )
        addressing_evidence = self.expect_list(
            addressing.get("evidence"), "CPU addressing matrix observer evidence"
        )
        self.expect_in(
            "focus_domain=cpu.addressing.page_cross_load",
            addressing_evidence,
        ...
        rmw_addressing = by_scenario.get("cpu_rmw_addressing_matrix_fault")
        if not isinstance(rmw_addressing, dict):
            self.errors.append("missing observer action for cpu_rmw_addressing_matrix_fault")
            return

        self.expect_equal(
            rmw_addressing.get("priority"),
            "known_divergence",
            "CPU RMW addressing matrix observer action priority",
        )
        self.expect_equal(
            rmw_addressing.get("action_type"),
            "inspect_known_divergence",
            "CPU RMW addressing matrix observer action type",
        )
        self.expect_equal(
            rmw_addressing.get("primary_artifact"),
            "cpu_rmw_addressing_matrix_fault/comparison.json",
            "CPU RMW addressing matrix observer primary_artifact",
        )
        rmw_addressing_evidence = self.expect_list(
            rmw_addressing.get("evidence"), "CPU RMW addressing matrix observer evidence"
        )
        self.expect_in(
            "focus_domain=cpu.rmw.absolute_asl",
            rmw_addressing_evidence,
            "CPU RMW addressing matrix observer evidence",
        )
        self.expect_in(
            "failed_probe_ids=cartridge.status.pass,cartridge.test.38.result",
            rmw_addressing_evidence,
            "CPU RMW addressing matrix observer evidence",
        )
    ...
    def verify_observer_observations(self, observations: list[Any]) -> None:
        ...
        if isinstance(ppu_increment_32, dict):
            self.expect_equal(
                ppu_increment_32.get("role"),
                "expected_failure_fixture",
                "PPU increment-32 observer role",
            )
            self.expect_equal(
                ppu_increment_32.get("outcome"),
                "expected_baseline_divergence",
                "PPU increment-32 observer outcome",
            )
            self.expect_equal(
                ppu_increment_32.get("health"),
                "cartridge_assertion_failed",
                "PPU increment-32 observer health",
            )
            self.expect_equal(
                ppu_increment_32.get("focus_domain"),
                "ppu.registers.ppudata_increment_32",
                "PPU increment-32 observer focus_domain",
            )
            self.expect_equal(
                ppu_increment_32.get("next_artifact"),
                "ppu_vram_increment_32_fault/comparison.json",
                "PPU increment-32 observer next_artifact",
            )
        else:
        ...
        if isinstance(addressing, dict):
            self.expect_equal(
                addressing.get("role"),
                "expected_failure_fixture",
                "CPU addressing matrix observer role",
            )
            self.expect_equal(
                addressing.get("outcome"),
                "expected_baseline_divergence",
                "CPU addressing matrix observer outcome",
            )
            self.expect_equal(
                addressing.get("health"),
                "cartridge_assertion_failed",
                "CPU addressing matrix observer health",
            )
            self.expect_equal(
                addressing.get("focus_domain"),
                "cpu.addressing.page_cross_load",
                "CPU addressing matrix observer focus_domain",
            )
            self.expect_equal(
                addressing.get("next_artifact"),
                "cpu_addressing_matrix_fault/comparison.json",
                "CPU addressing matrix observer next_artifact",
            )
        else:
        ...
        rmw_addressing = by_scenario.get("cpu_rmw_addressing_matrix_fault")
        if isinstance(rmw_addressing, dict):
            self.expect_equal(
                rmw_addressing.get("role"),
                "expected_failure_fixture",
                "CPU RMW addressing matrix observer role",
            )
            self.expect_equal(
                rmw_addressing.get("outcome"),
                "expected_baseline_divergence",
                "CPU RMW addressing matrix observer outcome",
            )
            self.expect_equal(
                rmw_addressing.get("health"),
                "cartridge_assertion_failed",
                "CPU RMW addressing matrix observer health",
            )
            self.expect_equal(
                rmw_addressing.get("focus_domain"),
                "cpu.rmw.absolute_asl",
                "CPU RMW addressing matrix observer focus_domain",
            )
            self.expect_equal(
                rmw_addressing.get("next_artifact"),
                "cpu_rmw_addressing_matrix_fault/comparison.json",
                "CPU RMW addressing matrix observer next_artifact",
            )
        else:
            self.errors.append("missing observer observation for cpu_rmw_addressing_matrix_fault")
