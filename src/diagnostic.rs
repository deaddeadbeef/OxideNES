...
pub const DIAGNOSTIC_TELEMETRY_SCHEMA_VERSION: u16 = 85;
pub const DIAGNOSTIC_SUITE_NAME: &str = "oxidenes_headless_diagnostic_cartridge";
pub const DIAGNOSTIC_SUITE_VERSION: &str = "diagnostic-cartridge-v85";
...
const MAPPER4_A12_GATE_IRQ_LATCH: u8 = 0x01;
const MAPPER4_A12_GATE_LOW_PATTERN_CTRL: u8 = 0x00;
const MAPPER4_A12_GATE_HIGH_PATTERN_CTRL: u8 = 0x10;
const MAPPER4_A12_GATE_BACKGROUND_MASK: u8 = 0x08;
...
const MAPPER4_MMC3_A12_GATE_PHASE_ADDR: u16 = 0x030C;
...
const CPU_LOAD_STORE_MATRIX_LOAD_MASK_ADDR: u16 = 0x02DE;
const CPU_LOAD_STORE_MATRIX_STORE_MASK_ADDR: u16 = 0x02DF;
const CPU_LOAD_STORE_MATRIX_TRANSFER_MASK_ADDR: u16 = 0x02E0;
const CPU_LOAD_STORE_MATRIX_LOAD_A_RESULT_ADDR: u16 = 0x02E1;
const CPU_LOAD_STORE_MATRIX_LOAD_X_RESULT_ADDR: u16 = 0x02E2;
const CPU_LOAD_STORE_MATRIX_LOAD_Y_RESULT_ADDR: u16 = 0x02E3;
const CPU_LOAD_STORE_MATRIX_STORE_A_RESULT_ADDR: u16 = 0x02E4;
const CPU_LOAD_STORE_MATRIX_STORE_X_RESULT_ADDR: u16 = 0x02E5;
const CPU_LOAD_STORE_MATRIX_STORE_Y_RESULT_ADDR: u16 = 0x02E6;
const CPU_LOAD_STORE_MATRIX_TRANSFER_TAX_RESULT_ADDR: u16 = 0x02E7;
const CPU_LOAD_STORE_MATRIX_TRANSFER_TAY_RESULT_ADDR: u16 = 0x02E8;
const CPU_LOAD_STORE_MATRIX_TRANSFER_TXA_RESULT_ADDR: u16 = 0x02E9;
...
const CPU_LOAD_STORE_MATRIX_LOAD_Y_ZP_X_RESULT_ADDR: u16 = 0x0257;
const CPU_LOAD_STORE_MATRIX_LOAD_X_ZP_RESULT_ADDR: u16 = 0x0258;
const CPU_LOAD_STORE_MATRIX_EXPECTED_LOAD_MASK: u8 = 0xFF;
const CPU_LOAD_STORE_MATRIX_EXPECTED_STORE_MASK: u8 = 0x7F;
const CPU_LOAD_STORE_MATRIX_EXPECTED_TRANSFER_MASK: u8 = 0x0F;
const CPU_LOAD_STORE_MATRIX_EXPECTED_CASE_COUNT: u8 = 19;
...
const CPU_ALU_INDEX_MATRIX_ORA_ZP_RESULT_ADDR: u16 = 0x025A;
const CPU_ALU_INDEX_MATRIX_EOR_ZP_RESULT_ADDR: u16 = 0x025B;
const CPU_ALU_INDEX_MATRIX_EXPECTED_LOGIC_MASK: u8 = 0x3F;
const CPU_ALU_INDEX_MATRIX_EXPECTED_INDEX_MASK: u8 = 0x0F;
const CPU_ALU_INDEX_MATRIX_EXPECTED_CASE_COUNT: u8 = 10;
...
const CPU_ARITHMETIC_MATRIX_ADC_CARRY_ZERO_RESULT_ADDR: u16 = 0x02F9;
const CPU_ARITHMETIC_MATRIX_ADC_CARRY_IN_RESULT_ADDR: u16 = 0x02FA;
const CPU_ARITHMETIC_MATRIX_ADC_NEG_OVERFLOW_RESULT_ADDR: u16 = 0x02FB;
const CPU_ARITHMETIC_MATRIX_SBC_NO_BORROW_RESULT_ADDR: u16 = 0x02FC;
const CPU_ARITHMETIC_MATRIX_SBC_BORROW_RESULT_ADDR: u16 = 0x02FD;
const CPU_ARITHMETIC_MATRIX_SBC_OVERFLOW_RESULT_ADDR: u16 = 0x02FE;
const CPU_ARITHMETIC_MATRIX_SBC_BORROW_IN_RESULT_ADDR: u16 = 0x02FF;
const CPU_ARITHMETIC_MATRIX_CASE_COUNT_ADDR: u16 = 0x0300;
...
pub const DIAGNOSTIC_TESTS: &[DiagnosticTestSpec] = &[
    ...
    DiagnosticTestSpec {
        id: CPU_LOAD_STORE_MATRIX_TEST_ID,
        name: "cpu_load_store_transfer_matrix",
        subsystem: DiagnosticSubsystem::Cpu,
        tier: DiagnosticTestTier::EdgeCase,
        ...
        expected_observations: &[
            "LDA covers indexed-indirect, absolute,Y, and indirect-indexed addressing while LDX and LDY cover zero-page indexed plus absolute indexed addressing with retained positive, zero, and negative flag outcomes",
    ...
    DiagnosticTestSpec {
        id: CPU_ARITHMETIC_MATRIX_TEST_ID,
        name: "cpu_arithmetic_flag_matrix",
        subsystem: DiagnosticSubsystem::Cpu,
        tier: DiagnosticTestTier::EdgeCase,
        intent: "Verify ADC and SBC preserve carry, borrow, overflow, zero, and negative flag behavior across representative arithmetic edge cases.",
        expected_observations: &[
...
const DIAGNOSTIC_FAILURES: &[DiagnosticFailureSpec] = &[
    ...
    DiagnosticFailureSpec {
        ...
        expected: "LDA indexed-indirect reads 0x5A, LDA absolute,Y can set zero, LDA indirect-indexed reads 0x91, LDX zero-page,Y reads 0x42, LDX zero-page reads 0x24, LDX absolute,Y can set negative, LDY zero-page,X reads 0x39, and LDY absolute,X reads 0x7E",
    ...
    DiagnosticFailureSpec {
        ...
        expected: "STA absolute,Y, STA absolute,X, STA zero-page,X, STA (zero-page,X), STA (zero-page),Y, STX zero-page,Y, and STY zero-page,X write the expected RAM sentinels",
    ...
    DiagnosticFailureSpec {
        code: 0xC3,
        test_id: CPU_LOAD_STORE_MATRIX_TEST_ID,
        assertion: "Transfer matrix covers TAX, TAY, TXA, and TYA result and flag outcomes",
        expected: "TAX, TAY, TXA, and TYA transfer register values and update zero/negative flags",
        observed: "one or more transfer opcodes did not preserve the expected register result or status flags",
        likely_domain: "cpu.load_store.transfer_matrix",
        remediation_hint: "Inspect transfer opcode register writeback and zero/negative flag updates.",
    },
    DiagnosticFailureSpec {
        code: 0xC4,
        test_id: CPU_LOAD_STORE_MATRIX_TEST_ID,
        assertion: "Load/store/transfer matrix records every opcode family subcase",
        expected: "load mask == 0xFF, store mask == 0x7F, transfer mask == 0x0F, and case count == 19",
    ...
    DiagnosticFailureSpec {
        ...
        expected: "AND immediate produces 0x00 with zero set, AND zero-page produces 0x88 with negative set, ORA immediate produces 0xC0 with negative set, ORA zero-page produces 0x18 with negative clear, EOR immediate produces 0x80 with negative set, and EOR zero-page produces 0x00 with zero set",
    ...
    DiagnosticFailureSpec {
        ...
        expected: "logic mask == 0x3F, index mask == 0x0F, and case count == 10",
    ...
    DiagnosticFailureSpec {
        code: 0xE3,
        test_id: CPU_ARITHMETIC_MATRIX_TEST_ID,
        assertion: "ADC arithmetic matrix covers carry-in, carry-out, overflow, zero, and negative flag outcomes",
        expected: "ADC produces 0x80, 0x00, 0x03, and 0x00 across the positive-overflow, carry-zero, carry-in, and negative-overflow cases",
        observed: "one or more ADC edge cases did not preserve the expected result or status flags",
        likely_domain: "cpu.arithmetic.adc_sbc_flags",
        remediation_hint: "Inspect ADC carry input, carry output, overflow calculation, accumulator writeback, and zero/negative flag updates.",
    },
    DiagnosticFailureSpec {
        code: 0xE4,
        test_id: CPU_ARITHMETIC_MATRIX_TEST_ID,
        assertion: "SBC arithmetic matrix covers borrow, no-borrow, overflow, zero, and negative flag outcomes",
        expected: "SBC produces 0x02, 0xFF, 0x7F, and 0x00 across the no-borrow, borrow, overflow, and borrow-in cases",
        observed: "one or more SBC edge cases did not preserve the expected result or status flags",
        likely_domain: "cpu.arithmetic.adc_sbc_flags",
        remediation_hint: "Inspect SBC carry-as-not-borrow semantics, overflow calculation, accumulator writeback, and zero/negative flag updates.",
    },
    DiagnosticFailureSpec {
        code: 0xE5,
        test_id: CPU_ARITHMETIC_MATRIX_TEST_ID,
        assertion: "Arithmetic matrix records every ADC/SBC subcase",
        expected: "ADC mask == 0x0F, SBC mask == 0x0F, and case count == 8",
        observed: "the arithmetic matrix did not record every expected ADC/SBC subcase",
        likely_domain: "cpu.arithmetic.adc_sbc_flags",
...
const DIAGNOSTIC_COVERAGE_GAPS: &[DiagnosticCoverageGapSpec] = &[
    ...
    DiagnosticCoverageGapSpec {
        ...
        current_coverage: "Joypad 1 and joypad 2 strobe/shift sequences use explicit expected masks, the scenario suite includes generated default, alternating, all-released, all-pressed, joypad-1-only pressed, joypad-2-only pressed, sparse-bits, and nibble-split input-mask pass fixtures, joypad 1 verifies mid-stream strobe reset behavior, a combined input-port matrix verifies both $4016 and $4017 strobe-high reads, full eight-bit serial masks, and overreads, a generated input-mask sweep variant reconstructs both serial bytes across 16 host-applied mask pairs, the e2e diagnostic runner writes diagnostic-input-sweep.json with an exhaustive 65,536-pair two-port Joypad core sweep, host input-remapping regression fixtures prove custom keyboard/controller bindings, turbo gates, merged sources, and joypad mask serialization through the same library path used by the game loop, a bus-level disconnected/default controller fixture proves both $4016 and $4017 serialize eight released bits before post-exhaustion reads go high, a headless host-event snapshot fixture injects keyboard, controller button, D-pad, and stick-derived events before proving the resulting masks through real bus serial reads, an OS-typed minifb/gilrs adapter fixture proves real minifb::Key and gilrs::Button values flow through host input mapping before bus serial reads, and a gilrs polling helper fixture proves D-pad button polling, analog stick cardinal snapping with hysteresis, turbo gates, trigger flags, and bus serial reads use the same library path as the game loop.",
        missing_coverage: "Live minifb Window key polling plus physical or virtual gilrs device enumeration and event pumping beyond typed adapter and closure-driven polling fixtures, plus in-cartridge exhaustive mask iteration beyond the sampled generated fixtures.",
        suggested_next_test: "Add a local live-input polling harness that opens a minifb window and uses a physical or virtual gilrs device to prove real polled window/controller events still serialize through $4016/$4017 correctly.",
    },
];
...
pub struct DiagnosticTelemetry {
    ...
    pub cpu_arithmetic_matrix: CpuArithmeticMatrixTelemetry,
    ...
    pub cpu_load_store_matrix: CpuLoadStoreMatrixTelemetry,
...
#[derive(Debug, Serialize)]
pub struct Mapper4Mmc3A12GateTelemetry {
    pub mapper: u8,
    pub prg_16k_banks: u8,
    pub chr_8k_banks: u8,
    pub irq_latch: u8,
    pub irq_latch_hex: String,
    pub low_pattern_ctrl: u8,
    pub low_pattern_ctrl_hex: String,
    pub high_pattern_ctrl: u8,
    pub high_pattern_ctrl_hex: String,
    pub background_mask: u8,
    pub background_mask_hex: String,
    pub expected_low_pattern_irq_count: u8,
...
#[derive(Debug, Serialize)]
pub struct CpuLoadStoreMatrixTelemetry {
    pub expected_case_count: u8,
    pub observed_case_count: u8,
    pub expected_load_mask: u8,
    pub expected_load_mask_hex: String,
    pub load_mask: u8,
    pub load_mask_hex: String,
    pub expected_store_mask: u8,
    pub expected_store_mask_hex: String,
    pub store_mask: u8,
    pub store_mask_hex: String,
    pub expected_transfer_mask: u8,
    pub expected_transfer_mask_hex: String,
    pub transfer_mask: u8,
    pub transfer_mask_hex: String,
    pub load_a_result: u8,
    pub load_a_result_hex: String,
    pub load_x_result: u8,
    pub load_x_result_hex: String,
    pub load_y_result: u8,
    ...
    pub load_y_zp_x_result: u8,
    pub load_y_zp_x_result_hex: String,
    pub load_x_zp_result: u8,
    pub load_x_zp_result_hex: String,
    pub store_a_result: u8,
    pub store_a_result_hex: String,
    pub store_x_result: u8,
    pub store_x_result_hex: String,
    pub store_y_result: u8,
    pub store_y_result_hex: String,
    pub transfer_tax_result: u8,
    pub transfer_tax_result_hex: String,
    pub transfer_tay_result: u8,
    pub transfer_tay_result_hex: String,
    pub transfer_txa_result: u8,
...
pub struct CpuAluIndexMatrixTelemetry {
    ...
    pub ora_zp_result: u8,
    pub ora_zp_result_hex: String,
    pub eor_result: u8,
    pub eor_result_hex: String,
    pub eor_zp_result: u8,
    pub eor_zp_result_hex: String,
    pub inx_result: u8,
...
#[derive(Debug, Serialize)]
pub struct CpuArithmeticMatrixTelemetry {
    pub expected_case_count: u8,
    pub observed_case_count: u8,
    pub expected_adc_mask: u8,
    pub expected_adc_mask_hex: String,
    pub adc_mask: u8,
    pub adc_mask_hex: String,
    pub expected_sbc_mask: u8,
    pub expected_sbc_mask_hex: String,
    pub sbc_mask: u8,
    pub sbc_mask_hex: String,
    pub adc_overflow_result: u8,
    pub adc_overflow_result_hex: String,
    pub adc_carry_zero_result: u8,
    pub adc_carry_zero_result_hex: String,
    pub adc_carry_in_result: u8,
    pub adc_carry_in_result_hex: String,
    pub adc_negative_overflow_result: u8,
    pub adc_negative_overflow_result_hex: String,
    pub sbc_no_borrow_result: u8,
    pub sbc_no_borrow_result_hex: String,
    pub sbc_borrow_result: u8,
    pub sbc_borrow_result_hex: String,
    pub sbc_overflow_result: u8,
...
fn build_mapper4_mmc3_a12_gate_variant_cartridge() -> Result<Vec<u8>, String> {
    let (program, labels) = build_mapper4_mmc3_a12_gate_variant_program_with_labels()?;
    if program.len() > 0x2000 {
        return Err(format!(
            "Mapper 4 A12 gate diagnostic program is too large for fixed $C000-$DFFF execution: {} bytes > {} bytes",
            program.len(),
            0x2000
        ));
    }
...
fn build_mapper4_mmc3_a12_gate_variant_program_with_labels(
) -> Result<(Vec<u8>, HashMap<String, u16>), String> {
    let mut program = DiagnosticProgram::new();
    ...
    program.asm.lda_imm(STATUS_RUNNING);
    program.asm.sta_zp(STATUS_ADDR);
    program.asm.lda_imm(MAPPER4_MMC3_A12_GATE_TEST_ID);
    program.asm.sta_zp(CURRENT_TEST_ADDR);
    program.asm.lda_imm(0xA5);
    program.asm.sta_zp(SIGNATURE_ADDR);
    program.asm.lda_imm(0x00);
    program.asm.sta_zp(FAILURE_CODE_ADDR);
    program.asm.sta_zp(NMI_COUNT_ADDR);
    program.asm.sta_abs(MAPPER4_MMC3_A12_GATE_PHASE_ADDR);
    program
        .asm
        .sta_abs(MAPPER4_MMC3_A12_GATE_LOW_IRQ_COUNT_ADDR);
    program
        .asm
        .sta_abs(MAPPER4_MMC3_A12_GATE_HIGH_IRQ_COUNT_ADDR);
    program.asm.sta_abs(MAPPER4_MMC3_A12_GATE_CASE_COUNT_ADDR);
    program.asm.sta_abs(0x2000);
    program.asm.sta_abs(0x2001);
    program.asm.sta_abs(0xE000);

    program.asm.lda_imm(MAPPER4_A12_GATE_IRQ_LATCH);
    program.asm.sta_abs(0xC000);
    program.asm.lda_imm(0x00);
    program.asm.sta_abs(0xC001);
    program.asm.sta_abs(0xE001);
    program.asm.lda_imm(0x01);
    program.asm.sta_abs(MAPPER4_MMC3_A12_GATE_PHASE_ADDR);
    program.asm.lda_imm(MAPPER4_A12_GATE_LOW_PATTERN_CTRL);
    program.asm.sta_abs(0x2000);
    program.asm.lda_imm(MAPPER4_A12_GATE_BACKGROUND_MASK);
    program.asm.sta_abs(0x2001);
    program.asm.cli();
    program.wait_for_vblank("mapper4_a12_low_vblank_1");
    program.wait_for_vblank("mapper4_a12_low_vblank_2");
    program.wait_for_vblank("mapper4_a12_low_vblank_3");
    program.asm.sei();
    program.asm.lda_imm(0x00);
    program.asm.sta_abs(0x2001);
    program.asm.sta_abs(0xE000);
    program
        .asm
        .lda_abs(MAPPER4_MMC3_A12_GATE_LOW_IRQ_COUNT_ADDR);
    program.expect_a_eq(MAPPER4_A12_GATE_EXPECTED_LOW_IRQ_COUNT, 0xEA);
    program.increment_abs(MAPPER4_MMC3_A12_GATE_CASE_COUNT_ADDR);
    ...
    program.asm.lda_imm(0x02);
    program.asm.sta_abs(MAPPER4_MMC3_A12_GATE_PHASE_ADDR);
    program.asm.lda_imm(MAPPER4_A12_GATE_HIGH_PATTERN_CTRL);
    program.asm.sta_abs(0x2000);
    program.asm.lda_imm(MAPPER4_A12_GATE_BACKGROUND_MASK);
    program.asm.sta_abs(0x2001);
    program.asm.cli();
    program.asm.label("wait_mapper4_a12_high_irq")?;
    program
        .asm
        .lda_abs(MAPPER4_MMC3_A12_GATE_HIGH_IRQ_COUNT_ADDR);
    program
        .asm
        .cmp_imm(MAPPER4_A12_GATE_EXPECTED_HIGH_IRQ_COUNT);
    program.asm.bne("wait_mapper4_a12_high_irq");
    program.asm.sei();
    program.asm.lda_imm(0x00);
    program.asm.sta_abs(0x2001);
    program.asm.sta_abs(0xE000);
    program
        .asm
        .lda_abs(MAPPER4_MMC3_A12_GATE_HIGH_IRQ_COUNT_ADDR);
    program.expect_a_eq(MAPPER4_A12_GATE_EXPECTED_HIGH_IRQ_COUNT, 0xEB);
    program.increment_abs(MAPPER4_MMC3_A12_GATE_CASE_COUNT_ADDR);
    ...
    program.asm.lda_imm(0x00);
    program.asm.sta_abs(0xE000);
    program.asm.lda_abs(MAPPER4_MMC3_A12_GATE_PHASE_ADDR);
    program.asm.cmp_imm(0x01);
    program.asm.beq("mapper4_a12_low_irq");
    program.asm.cmp_imm(0x02);
    program.asm.beq("mapper4_a12_high_irq");
    program.asm.rti();
    program.asm.label("mapper4_a12_low_irq")?;
    program
        .asm
        .inc_abs(MAPPER4_MMC3_A12_GATE_LOW_IRQ_COUNT_ADDR);
    program.asm.rti();
    program.asm.label("mapper4_a12_high_irq")?;
    program
        .asm
        .inc_abs(MAPPER4_MMC3_A12_GATE_HIGH_IRQ_COUNT_ADDR);
    program.asm.rti();
    program.asm.label("hang")?;
    program.asm.jmp_label("hang");
...
pub fn run_diagnostic(config: DiagnosticConfig) -> Result<DiagnosticTelemetry, String> {
    ...
    let mut host_failures = host_validate(HostValidationInput {
        ...
        cpu_load_store_matrix: &cpu_load_store_matrix,
...
fn write_mapper_section(report: &mut String, telemetry: &DiagnosticTelemetry) {
    ...
    writeln!(
        report,
        "| Mapper 4 A12 gate PPUCTRL low / high / PPUMASK | {} / {} / {} |",
        telemetry.mapper4_mmc3_a12_gate.low_pattern_ctrl_hex,
        telemetry.mapper4_mmc3_a12_gate.high_pattern_ctrl_hex,
        telemetry.mapper4_mmc3_a12_gate.background_mask_hex
    )
    .expect("write report");
    writeln!(
        report,
        "| Mapper 4 A12 gate IRQ latch | {} |",
        telemetry.mapper4_mmc3_a12_gate.irq_latch_hex
    )
    .expect("write report");
    writeln!(
        report,
        "| Mapper 4 A12 gate low-pattern IRQ observed / expected | {} / {} |",
        telemetry
            .mapper4_mmc3_a12_gate
    ...
    writeln!(
        report,
        "| Mapper 4 A12 gate high-pattern IRQ observed / expected | {} / {} |",
        telemetry
            .mapper4_mmc3_a12_gate
    ...
    writeln!(
        report,
        "| Mapper 4 A12 gate cases / expected | {} / {} |",
        telemetry.mapper4_mmc3_a12_gate.observed_case_count,
        telemetry.mapper4_mmc3_a12_gate.expected_case_count
    )
    .expect("write report");
    writeln!(
        report,
        "| Mapper 4 A12 gate cycles / frames / passed | {} / {} / {} |",
        telemetry.mapper4_mmc3_a12_gate.cycles,
        telemetry.mapper4_mmc3_a12_gate.frames,
        telemetry.mapper4_mmc3_a12_gate.passed
    )
    .expect("write report");
    writeln!(
        report,
...
fn write_cpu_load_store_section(report: &mut String, telemetry: &DiagnosticTelemetry) {
    writeln!(report, "## CPU Load/Store/Transfer Matrix").expect("write report");
    writeln!(report).expect("write report");
    writeln!(report, "| Field | Value |").expect("write report");
    writeln!(report, "| --- | --- |").expect("write report");
    writeln!(
        report,
        "| Load/store/transfer masks | {} / {} / {} |",
        telemetry.cpu_load_store_matrix.load_mask_hex,
        telemetry.cpu_load_store_matrix.store_mask_hex,
        telemetry.cpu_load_store_matrix.transfer_mask_hex
    )
    .expect("write report");
    writeln!(
        report,
        "| Expected masks | {} / {} / {} |",
        telemetry.cpu_load_store_matrix.expected_load_mask_hex,
        telemetry.cpu_load_store_matrix.expected_store_mask_hex,
        telemetry.cpu_load_store_matrix.expected_transfer_mask_hex
    )
    .expect("write report");
    writeln!(
        report,
        "| Load A/X/Y/LDA (zp),Y/LDX zp,Y/LDY zp,X/LDX zp results | {} / {} / {} / {} / {} / {} / {} |",
        ...
        telemetry.cpu_load_store_matrix.load_x_zp_result_hex
    )
    .expect("write report");
    writeln!(
        report,
        "| Store A/X/Y results | {} / {} / {} |",
        telemetry.cpu_load_store_matrix.store_a_result_hex,
        telemetry.cpu_load_store_matrix.store_x_result_hex,
        telemetry.cpu_load_store_matrix.store_y_result_hex
    )
    .expect("write report");
    writeln!(
        report,
        "| Transfer TAX/TAY/TXA/TYA results | {} / {} / {} / {} |",
    ...
    writeln!(
        report,
        "| Load/store/transfer cases / expected | {} / {} |",
        telemetry.cpu_load_store_matrix.observed_case_count,
        telemetry.cpu_load_store_matrix.expected_case_count
    )
    .expect("write report");
    writeln!(
        report,
        "| Load/store/transfer matrix passed | {} |",
        telemetry.cpu_load_store_matrix.passed
    )
    .expect("write report");
    writeln!(report).expect("write report");
}
...
fn write_cpu_alu_index_section(report: &mut String, telemetry: &DiagnosticTelemetry) {
    ...
    writeln!(
        report,
        "| AND imm/AND zp/ORA imm/ORA zp/EOR imm/EOR zp results | {} / {} / {} / {} / {} / {} |",
        ...
        telemetry.cpu_alu_index_matrix.ora_zp_result_hex,
        telemetry.cpu_alu_index_matrix.eor_result_hex,
        telemetry.cpu_alu_index_matrix.eor_zp_result_hex
    )
    .expect("write report");
    writeln!(
        report,
...
fn write_cpu_arithmetic_section(report: &mut String, telemetry: &DiagnosticTelemetry) {
    writeln!(report, "## CPU Arithmetic Flag Matrix").expect("write report");
    writeln!(report).expect("write report");
    writeln!(report, "| Field | Value |").expect("write report");
    writeln!(report, "| --- | --- |").expect("write report");
    writeln!(
        report,
        "| ADC/SBC masks | {} / {} |",
        telemetry.cpu_arithmetic_matrix.adc_mask_hex, telemetry.cpu_arithmetic_matrix.sbc_mask_hex
    )
    .expect("write report");
    writeln!(
        report,
        "| Expected masks | {} / {} |",
        telemetry.cpu_arithmetic_matrix.expected_adc_mask_hex,
        telemetry.cpu_arithmetic_matrix.expected_sbc_mask_hex
    )
    .expect("write report");
    writeln!(
        report,
        "| ADC overflow/carry-zero/carry-in/negative-overflow results | {} / {} / {} / {} |",
    ...
    writeln!(
        report,
        "| SBC no-borrow/borrow/overflow/borrow-in results | {} / {} / {} / {} |",
    ...
    writeln!(
        report,
        "| Arithmetic cases / expected | {} / {} |",
        telemetry.cpu_arithmetic_matrix.observed_case_count,
        telemetry.cpu_arithmetic_matrix.expected_case_count
    )
    .expect("write report");
    writeln!(
        report,
        "| Arithmetic matrix passed | {} |",
        telemetry.cpu_arithmetic_matrix.passed
    )
    .expect("write report");
    writeln!(report).expect("write report");
}
...
impl OpcodeDecode {
    ...
    fn zero_page_y(mnemonic: &'static str) -> Self {
        Self {
            mnemonic,
    ...
    fn absolute_y(mnemonic: &'static str) -> Self {
        Self {
            mnemonic,
    ...
    fn indexed_indirect(mnemonic: &'static str) -> Self {
        Self {
            mnemonic,
...
fn format_instruction_text(decode: OpcodeDecode, pc: u16, operand_bytes: &[u8]) -> String {
    match decode.addressing_mode {
        ...
        "zero_page_y" => format!("{} ${:02X},Y", decode.mnemonic, operand_bytes[0]),
        "absolute" => format!(
            "{} {}",
            decode.mnemonic,
        ...
        "indexed_indirect" => format!("{} (${:02X},X)", decode.mnemonic, operand_bytes[0]),
...
struct HostValidationInput<'a> {
    ...
    cpu_load_store_matrix: &'a CpuLoadStoreMatrixTelemetry,
...
fn host_validate(input: HostValidationInput<'_>) -> Vec<String> {
    ...
    if !input.cpu_load_store_matrix.passed {
        failures.push(format!(
            "CPU load/store/transfer matrix mismatch: load_mask={}, store_mask={}, transfer_mask={}, load A/X/Y/LDA (zp),Y/LDX zp,Y/LDY zp,X/LDX zp={}/{}/{}/{}/{}/{}/{}, store A/X/Y={}/{}/{}, transfer TAX/TAY/TXA/TYA={}/{}/{}/{}, cases {}/{}",
            ...
            input.cpu_load_store_matrix.load_a_result_hex,
            ...
            input.cpu_load_store_matrix.load_y_zp_x_result_hex,
            input.cpu_load_store_matrix.load_x_zp_result_hex,
            ...
            input.cpu_load_store_matrix.transfer_tax_result_hex,
            input.cpu_load_store_matrix.transfer_tay_result_hex,
            input.cpu_load_store_matrix.transfer_txa_result_hex,
    ...
    if !input.cpu_alu_index_matrix.passed {
        failures.push(format!(
            "CPU ALU/index matrix mismatch: logic_mask={}, index_mask={}, AND imm/AND zp/ORA imm/ORA zp/EOR imm/EOR zp={}/{}/{}/{}/{}/{}, INX/INY/DEX/DEY={}/{}/{}/{}, cases {}/{}",
            ...
            input.cpu_alu_index_matrix.ora_zp_result_hex,
            input.cpu_alu_index_matrix.eor_result_hex,
            input.cpu_alu_index_matrix.eor_zp_result_hex,
    ...
    if !input.cpu_arithmetic_matrix.passed {
        failures.push(format!(
            "CPU arithmetic matrix mismatch: adc_mask={}, sbc_mask={}, ADC overflow/carry_zero/carry_in/negative_overflow={}/{}/{}/{}, SBC no_borrow/borrow/overflow/borrow_in={}/{}/{}/{}, cases {}/{}",
            input.cpu_arithmetic_matrix.adc_mask_hex,
            input.cpu_arithmetic_matrix.sbc_mask_hex,
            input.cpu_arithmetic_matrix.adc_overflow_result_hex,
            input.cpu_arithmetic_matrix.adc_carry_zero_result_hex,
            input.cpu_arithmetic_matrix.adc_carry_in_result_hex,
            input.cpu_arithmetic_matrix.adc_negative_overflow_result_hex,
            input.cpu_arithmetic_matrix.sbc_no_borrow_result_hex,
            input.cpu_arithmetic_matrix.sbc_borrow_result_hex,
            input.cpu_arithmetic_matrix.sbc_overflow_result_hex,
    ...
    if !input.mapper4_mmc3_a12_gate.passed {
        failures.push(format!(
            "Mapper 4 MMC3 A12 gate variant mismatch: low-pattern IRQ {}/{}, high-pattern IRQ {}/{}, PPUCTRL low/high {}/{}, PPUMASK {}, cases {}/{}, cycles={}, error {}",
            input.mapper4_mmc3_a12_gate.observed_low_pattern_irq_count,
            input.mapper4_mmc3_a12_gate.expected_low_pattern_irq_count,
            input.mapper4_mmc3_a12_gate.observed_high_pattern_irq_count,
            input.mapper4_mmc3_a12_gate.expected_high_pattern_irq_count,
            input.mapper4_mmc3_a12_gate.low_pattern_ctrl_hex,
            input.mapper4_mmc3_a12_gate.high_pattern_ctrl_hex,
            input.mapper4_mmc3_a12_gate.background_mask_hex,
            input.mapper4_mmc3_a12_gate.observed_case_count,
...
fn probe_telemetry(input: ProbeTelemetryInput<'_>) -> Vec<DiagnosticProbeTelemetry> {
    ...
    push_probe(
        &mut probes,
        ProbeTelemetryRecord {
            id: "cpu.load_store_matrix.results".to_string(),
            source: DiagnosticProbeSource::HostObservation,
            subsystem: Some(DiagnosticSubsystem::Cpu),
            test_id: Some(CPU_LOAD_STORE_MATRIX_TEST_ID),
            test_name: test_name(CPU_LOAD_STORE_MATRIX_TEST_ID),
            status: gated_probe_status(passed_suite, input.cpu_load_store_matrix.passed),
            description:
                "CPU load/store/transfer matrix retained expected register flags and memory side effects"
                    .to_string(),
            expected: "load=0xFF, store=0x7F, transfer=0x0F, load A/X/Y/LDA (zp),Y/LDX zp,Y/LDY zp,X/LDX zp=0x5A/0x80/0x7E/0x91/0x42/0x39/0x24, store A/X/Y=0xC3/0x5E/0xA7, transfer TAX/TAY/TXA/TYA=0x44/0x00/0x80/0x7F, cases=19"
                .to_string(),
            observed: format!(
                "load {}, store {}, transfer {}, load A/X/Y/LDA (zp),Y/LDX zp,Y/LDY zp,X/LDX zp {}/{}/{}/{}/{}/{}/{}, store A/X/Y {}/{}/{}, transfer TAX/TAY/TXA/TYA {}/{}/{}/{}, cases {}/{}",
                input.cpu_load_store_matrix.load_mask_hex,
                input.cpu_load_store_matrix.store_mask_hex,
                input.cpu_load_store_matrix.transfer_mask_hex,
                input.cpu_load_store_matrix.load_a_result_hex,
        ...
        ProbeTelemetryRecord {
            ...
            description:
                "CPU logical ALU and index-register matrix retained expected result and status-flag observations"
                    .to_string(),
            expected:
                "logic=0x3F, index=0x0F, AND imm/AND zp/ORA imm/ORA zp/EOR imm/EOR zp=0x00/0x88/0xC0/0x18/0x80/0x00, INX/INY/DEX/DEY=0x00/0x80/0xFF/0x00, cases=10"
                    .to_string(),
            observed: format!(
                "logic {}, index {}, AND imm/AND zp/ORA imm/ORA zp/EOR imm/EOR zp {}/{}/{}/{}/{}/{}, INX/INY/DEX/DEY {}/{}/{}/{}, cases {}/{}",
                input.cpu_alu_index_matrix.logic_mask_hex,
                input.cpu_alu_index_matrix.index_mask_hex,
                input.cpu_alu_index_matrix.and_result_hex,
                input.cpu_alu_index_matrix.and_zp_result_hex,
    ...
    push_probe(
        &mut probes,
        ProbeTelemetryRecord {
            id: "cpu.arithmetic_matrix.results".to_string(),
            source: DiagnosticProbeSource::HostObservation,
            subsystem: Some(DiagnosticSubsystem::Cpu),
            test_id: Some(CPU_ARITHMETIC_MATRIX_TEST_ID),
            test_name: test_name(CPU_ARITHMETIC_MATRIX_TEST_ID),
            status: gated_probe_status(passed_suite, input.cpu_arithmetic_matrix.passed),
            description:
                "CPU ADC/SBC arithmetic matrix retained expected result and carry/overflow/zero/negative flag observations"
                    .to_string(),
            expected:
                "adc=0x0F, sbc=0x0F, ADC overflow/carry-zero/carry-in/negative-overflow=0x80/0x00/0x03/0x00, SBC no-borrow/borrow/overflow/borrow-in=0x02/0xFF/0x7F/0x00, cases=8"
                    .to_string(),
            observed: format!(
                "adc {}, sbc {}, ADC overflow/carry-zero/carry-in/negative-overflow {}/{}/{}/{}, SBC no-borrow/borrow/overflow/borrow-in {}/{}/{}/{}, cases {}/{}",
                input.cpu_arithmetic_matrix.adc_mask_hex,
                input.cpu_arithmetic_matrix.sbc_mask_hex,
                input.cpu_arithmetic_matrix.adc_overflow_result_hex,
                input.cpu_arithmetic_matrix.adc_carry_zero_result_hex,
    ...
    push_probe(
        &mut probes,
        ProbeTelemetryRecord {
            id: "mapper4.mmc3_a12_irq_gate".to_string(),
            source: DiagnosticProbeSource::HostObservation,
            subsystem: Some(DiagnosticSubsystem::Cartridge),
            test_id: Some(MAPPER4_MMC3_A12_GATE_TEST_ID),
            test_name: None,
            status: gated_probe_status(passed_suite, input.mapper4_mmc3_a12_gate.passed),
            description:
                "Generated Mapper 4 A12 gate variant proves low-pattern background rendering does not clock MMC3 IRQs while high-pattern rendering still does"
                    .to_string(),
            expected: format!(
                ...
                input.mapper4_mmc3_a12_gate.high_pattern_ctrl_hex,
                input.mapper4_mmc3_a12_gate.background_mask_hex,
                input.mapper4_mmc3_a12_gate.irq_latch_hex,
                input.mapper4_mmc3_a12_gate.expected_low_pattern_irq_count,
                input.mapper4_mmc3_a12_gate.expected_high_pattern_irq_count,
                input.mapper4_mmc3_a12_gate.expected_case_count
            ),
            observed: format!(
                ...
                input.mapper4_mmc3_a12_gate.expected_case_count,
                input.mapper4_mmc3_a12_gate.cycles,
                input.mapper4_mmc3_a12_gate.frames,
                optional_string(input.mapper4_mmc3_a12_gate.error.as_deref())
            ),
...
impl DiagnosticProgram {
    ...
    fn expect_v_clear(&mut self, fail_code: u8) {
        let ok = self.unique_label("overflow_clear_ok");
        self.asm.bvc(&ok);
        self.fail_with_code(fail_code);
        self.asm
            .label(&ok)
    ...
    fn expect_v_set(&mut self, fail_code: u8) {
        let ok = self.unique_label("overflow_set_ok");
        self.asm.bvs(&ok);
        self.fail_with_code(fail_code);
        self.asm
            .label(&ok)
    ...
    fn mark_load_store_matrix_case(&mut self, mask_addr: u16, mask_bit: u8) {
        self.asm.lda_abs(mask_addr);
        self.asm.ora_imm(mask_bit);
        self.asm.sta_abs(mask_addr);
    ...
    fn mark_arithmetic_matrix_case(&mut self, mask_addr: u16, mask_bit: u8) {
        self.asm.lda_abs(mask_addr);
        self.asm.ora_imm(mask_bit);
        self.asm.sta_abs(mask_addr);
    ...
    fn cpu_load_store_transfer_matrix(&mut self) {
        self.begin_test(CPU_LOAD_STORE_MATRIX_TEST_ID);
        self.asm.lda_imm(0x00);
        self.asm.sta_abs(CPU_LOAD_STORE_MATRIX_LOAD_MASK_ADDR);
        self.asm.sta_abs(CPU_LOAD_STORE_MATRIX_STORE_MASK_ADDR);
        self.asm.sta_abs(CPU_LOAD_STORE_MATRIX_TRANSFER_MASK_ADDR);
        self.asm.sta_abs(CPU_LOAD_STORE_MATRIX_LOAD_A_RESULT_ADDR);
        self.asm.sta_abs(CPU_LOAD_STORE_MATRIX_LOAD_X_RESULT_ADDR);
        ...
        self.asm
            .sta_abs(CPU_LOAD_STORE_MATRIX_LOAD_Y_ZP_X_RESULT_ADDR);
        self.asm
            .sta_abs(CPU_LOAD_STORE_MATRIX_LOAD_X_ZP_RESULT_ADDR);
        ...
        self.asm
            .sta_abs(CPU_LOAD_STORE_MATRIX_TRANSFER_TAX_RESULT_ADDR);
        self.asm
            .sta_abs(CPU_LOAD_STORE_MATRIX_TRANSFER_TAY_RESULT_ADDR);
        self.asm
            .sta_abs(CPU_LOAD_STORE_MATRIX_TRANSFER_TXA_RESULT_ADDR);
        self.asm
            .sta_abs(CPU_LOAD_STORE_MATRIX_TRANSFER_TYA_RESULT_ADDR);
        self.asm.sta_abs(CPU_LOAD_STORE_MATRIX_CASE_COUNT_ADDR);

        self.asm
            .label(CPU_LOAD_STORE_MATRIX_FAULT_LABEL)
            .expect("diagnostic fault-injection label should not collide");

        self.asm.lda_imm(0x60);
        self.asm.sta_zp(0x24);
        self.asm.lda_imm(0x04);
        self.asm.sta_zp(0x25);
        self.asm.lda_imm(0x5A);
        self.asm.sta_abs(0x0460);
        self.asm.ldx_imm(0x04);
        self.asm.lda_indexed_indirect(0x20);
        self.asm.sta_abs(CPU_LOAD_STORE_MATRIX_LOAD_A_RESULT_ADDR);
        self.expect_z_clear(0xBF);
        self.expect_n_clear(0xBF);
        self.expect_a_eq(0x5A, 0xBF);
        self.mark_load_store_matrix_case(CPU_LOAD_STORE_MATRIX_LOAD_MASK_ADDR, 0x01);

        self.asm.lda_imm(0x70);
        self.asm.sta_zp(0x5A);
        self.asm.lda_imm(0x05);
        self.asm.sta_zp(0x5B);
        self.asm.lda_imm(0x91);
        self.asm.sta_abs(0x0573);
        self.asm.ldy_imm(0x03);
        self.asm.lda_indirect_y(0x5A);
        self.asm
            .sta_abs(CPU_LOAD_STORE_MATRIX_LOAD_INDIRECT_Y_RESULT_ADDR);
        self.expect_z_clear(0xBF);
        ...
        self.asm.lda_imm(0x39);
        self.asm.sta_zp(0x4B);
        self.asm.ldx_imm(0x09);
        self.asm.ldy_zp_x(0x42);
        self.expect_z_clear(0xBF);
        self.expect_n_clear(0xBF);
        self.asm
            .sty_abs(CPU_LOAD_STORE_MATRIX_LOAD_Y_ZP_X_RESULT_ADDR);
        self.asm.cpy_imm(0x39);
        self.expect_z_set(0xBF);
        self.mark_load_store_matrix_case(CPU_LOAD_STORE_MATRIX_LOAD_MASK_ADDR, 0x40);

        self.asm.lda_imm(0x24);
        self.asm.sta_zp(0x34);
        self.asm.ldx_zp(0x34);
        self.expect_z_clear(0xBF);
        self.expect_n_clear(0xBF);
        self.asm
            .stx_abs(CPU_LOAD_STORE_MATRIX_LOAD_X_ZP_RESULT_ADDR);
        self.asm.cpx_imm(0x24);
        self.expect_z_set(0xBF);
        self.mark_load_store_matrix_case(CPU_LOAD_STORE_MATRIX_LOAD_MASK_ADDR, 0x80);

        self.asm.lda_imm(0x00);
        self.asm.sta_abs(0x0500);
        self.asm.ldy_imm(0x01);
        self.asm.lda_abs_y(0x04FF);
        self.expect_z_set(0xBF);
        self.expect_n_clear(0xBF);
        self.mark_load_store_matrix_case(CPU_LOAD_STORE_MATRIX_LOAD_MASK_ADDR, 0x02);

        self.asm.lda_imm(0x80);
        self.asm.sta_abs(0x0501);
        self.asm.ldy_imm(0x02);
        self.asm.ldx_abs_y(0x04FF);
        self.expect_z_clear(0xBF);
        self.expect_n_set(0xBF);
        self.asm.stx_abs(CPU_LOAD_STORE_MATRIX_LOAD_X_RESULT_ADDR);
        self.asm.cpx_imm(0x80);
        self.expect_z_set(0xBF);
        self.mark_load_store_matrix_case(CPU_LOAD_STORE_MATRIX_LOAD_MASK_ADDR, 0x04);

        self.asm.lda_imm(0x7E);
        self.asm.sta_abs(0x0502);
        self.asm.ldx_imm(0x05);
        self.asm.ldy_abs_x(0x04FD);
        self.expect_z_clear(0xBF);
        self.expect_n_clear(0xBF);
        self.asm.sty_abs(CPU_LOAD_STORE_MATRIX_LOAD_Y_RESULT_ADDR);
        self.asm.cpy_imm(0x7E);
        self.expect_z_set(0xBF);
        self.mark_load_store_matrix_case(CPU_LOAD_STORE_MATRIX_LOAD_MASK_ADDR, 0x08);

        self.asm.lda_imm(0xC3);
        self.asm.ldy_imm(0x11);
        self.asm.sta_abs_y(0x0500);
        self.asm.lda_abs(0x0511);
        self.asm.sta_abs(CPU_LOAD_STORE_MATRIX_STORE_A_RESULT_ADDR);
        self.expect_a_eq(0xC3, 0xC2);
        self.mark_load_store_matrix_case(CPU_LOAD_STORE_MATRIX_STORE_MASK_ADDR, 0x01);

        self.asm.ldx_imm(0x5E);
        self.asm.ldy_imm(0x04);
        self.asm.stx_zp_y(0x3C);
        self.asm.lda_zp(0x40);
        self.asm.sta_abs(CPU_LOAD_STORE_MATRIX_STORE_X_RESULT_ADDR);
        self.expect_a_eq(0x5E, 0xC2);
        self.mark_load_store_matrix_case(CPU_LOAD_STORE_MATRIX_STORE_MASK_ADDR, 0x02);

        self.asm.ldy_imm(0xA7);
        self.asm.ldx_imm(0x05);
        self.asm.sty_zp_x(0x3C);
        self.asm.lda_zp(0x41);
        self.asm.sta_abs(CPU_LOAD_STORE_MATRIX_STORE_Y_RESULT_ADDR);
        self.expect_a_eq(0xA7, 0xC2);
        self.mark_load_store_matrix_case(CPU_LOAD_STORE_MATRIX_STORE_MASK_ADDR, 0x04);

        self.asm.lda_imm(0x00);
        self.asm.sta_abs(0x0528);
        self.asm.lda_imm(0x3D);
        self.asm.ldx_imm(0x08);
        self.asm.sta_abs_x(0x0520);
        self.asm.lda_abs(0x0528);
        self.expect_a_eq(0x3D, 0xC2);
        self.mark_load_store_matrix_case(CPU_LOAD_STORE_MATRIX_STORE_MASK_ADDR, 0x08);

        self.asm.lda_imm(0x00);
        self.asm.sta_zp(0x47);
        self.asm.lda_imm(0x6B);
        self.asm.ldx_imm(0x05);
        self.asm.sta_zp_x(0x42);
        self.asm.lda_zp(0x47);
        ...
        self.asm.lda_imm(0x40);
        self.asm.sta_zp(0x58);
        self.asm.lda_imm(0x05);
        self.asm.sta_zp(0x59);
        self.asm.lda_imm(0x00);
        self.asm.sta_abs(0x0547);
        self.asm.lda_imm(0xE4);
        self.asm.ldy_imm(0x07);
        self.asm.sta_indirect_y(0x58);
        self.asm.lda_abs(0x0547);
        self.expect_a_eq(0xE4, 0xC2);
        self.mark_load_store_matrix_case(CPU_LOAD_STORE_MATRIX_STORE_MASK_ADDR, 0x40);

        self.asm.lda_imm(0x44);
        self.asm.tax();
        self.expect_z_clear(0xC3);
        self.expect_n_clear(0xC3);
        self.asm
            .stx_abs(CPU_LOAD_STORE_MATRIX_TRANSFER_TAX_RESULT_ADDR);
        self.asm.cpx_imm(0x44);
        self.expect_z_set(0xC3);
        self.mark_load_store_matrix_case(CPU_LOAD_STORE_MATRIX_TRANSFER_MASK_ADDR, 0x01);
        ...
        self.expect_n_clear(0xC3);
        self.asm
            .sty_abs(CPU_LOAD_STORE_MATRIX_TRANSFER_TAY_RESULT_ADDR);
        self.asm.cpy_imm(0x00);
        self.expect_z_set(0xC3);
        self.mark_load_store_matrix_case(CPU_LOAD_STORE_MATRIX_TRANSFER_MASK_ADDR, 0x02);

        self.asm.ldx_imm(0x80);
        self.asm.txa();
        self.asm
            .sta_abs(CPU_LOAD_STORE_MATRIX_TRANSFER_TXA_RESULT_ADDR);
        self.expect_z_clear(0xC3);
        self.expect_n_set(0xC3);
        self.expect_a_eq(0x80, 0xC3);
        self.mark_load_store_matrix_case(CPU_LOAD_STORE_MATRIX_TRANSFER_MASK_ADDR, 0x04);
        ...
        self.expect_z_clear(0xC3);
        self.expect_n_clear(0xC3);
        self.expect_a_eq(0x7F, 0xC3);
        self.mark_load_store_matrix_case(CPU_LOAD_STORE_MATRIX_TRANSFER_MASK_ADDR, 0x08);

        self.asm
            .label(CPU_LOAD_STORE_MATRIX_SUMMARY_LABEL)
            .expect("diagnostic load/store summary label should not collide");
        self.asm.lda_abs(CPU_LOAD_STORE_MATRIX_LOAD_MASK_ADDR);
        self.expect_a_eq(CPU_LOAD_STORE_MATRIX_EXPECTED_LOAD_MASK, 0xC4);
        self.asm.lda_abs(CPU_LOAD_STORE_MATRIX_STORE_MASK_ADDR);
        self.expect_a_eq(CPU_LOAD_STORE_MATRIX_EXPECTED_STORE_MASK, 0xC4);
        self.asm.lda_abs(CPU_LOAD_STORE_MATRIX_TRANSFER_MASK_ADDR);
        self.expect_a_eq(CPU_LOAD_STORE_MATRIX_EXPECTED_TRANSFER_MASK, 0xC4);
    ...
    fn cpu_alu_index_matrix(&mut self) {
        ...
        self.asm.sta_abs(CPU_ALU_INDEX_MATRIX_ORA_ZP_RESULT_ADDR);
        self.asm.sta_abs(CPU_ALU_INDEX_MATRIX_EOR_RESULT_ADDR);
        self.asm.sta_abs(CPU_ALU_INDEX_MATRIX_EOR_ZP_RESULT_ADDR);
        ...
        self.asm.lda_imm(0x10);
        self.asm.sta_zp(0x37);
        self.asm.lda_imm(0x08);
        self.asm.ora_zp(0x37);
        self.asm.sta_abs(CPU_ALU_INDEX_MATRIX_ORA_ZP_RESULT_ADDR);
        self.expect_z_clear(0xC5);
        self.expect_n_clear(0xC5);
        self.expect_a_eq(0x18, 0xC5);
        self.mark_alu_index_matrix_case(CPU_ALU_INDEX_MATRIX_LOGIC_MASK_ADDR, 0x10);
        ...
        self.asm.sta_zp(0x38);
        self.asm.lda_imm(0x3C);
        self.asm.eor_zp(0x38);
        self.asm.sta_abs(CPU_ALU_INDEX_MATRIX_EOR_ZP_RESULT_ADDR);
        self.expect_z_set(0xC5);
        self.expect_n_clear(0xC5);
        self.expect_a_eq(0x00, 0xC5);
        self.mark_alu_index_matrix_case(CPU_ALU_INDEX_MATRIX_LOGIC_MASK_ADDR, 0x20);
    ...
    fn cpu_arithmetic_flag_matrix(&mut self) {
        self.begin_test(CPU_ARITHMETIC_MATRIX_TEST_ID);
        self.asm.lda_imm(0x00);
        self.asm.sta_abs(CPU_ARITHMETIC_MATRIX_ADC_MASK_ADDR);
        self.asm.sta_abs(CPU_ARITHMETIC_MATRIX_SBC_MASK_ADDR);
        self.asm
            .sta_abs(CPU_ARITHMETIC_MATRIX_ADC_OVERFLOW_RESULT_ADDR);
        self.asm
            .sta_abs(CPU_ARITHMETIC_MATRIX_ADC_CARRY_ZERO_RESULT_ADDR);
        self.asm
            .sta_abs(CPU_ARITHMETIC_MATRIX_ADC_CARRY_IN_RESULT_ADDR);
        self.asm
            .sta_abs(CPU_ARITHMETIC_MATRIX_ADC_NEG_OVERFLOW_RESULT_ADDR);
        self.asm
            .sta_abs(CPU_ARITHMETIC_MATRIX_SBC_NO_BORROW_RESULT_ADDR);
        self.asm
            .sta_abs(CPU_ARITHMETIC_MATRIX_SBC_BORROW_RESULT_ADDR);
        self.asm
            .sta_abs(CPU_ARITHMETIC_MATRIX_SBC_OVERFLOW_RESULT_ADDR);
        self.asm
            .sta_abs(CPU_ARITHMETIC_MATRIX_SBC_BORROW_IN_RESULT_ADDR);
        self.asm.sta_abs(CPU_ARITHMETIC_MATRIX_CASE_COUNT_ADDR);

        self.asm
            .label(CPU_ARITHMETIC_MATRIX_FAULT_LABEL)
            .expect("diagnostic fault-injection label should not collide");
        ...
        self.asm
            .sta_abs(CPU_ARITHMETIC_MATRIX_ADC_OVERFLOW_RESULT_ADDR);
        self.expect_c_clear(0xE3);
        self.expect_v_set(0xE3);
        self.expect_z_clear(0xE3);
        self.expect_n_set(0xE3);
        self.expect_a_eq(0x80, 0xE3);
        self.mark_arithmetic_matrix_case(CPU_ARITHMETIC_MATRIX_ADC_MASK_ADDR, 0x01);
        ...
        self.asm
            .sta_abs(CPU_ARITHMETIC_MATRIX_ADC_CARRY_ZERO_RESULT_ADDR);
        self.expect_c_set(0xE3);
        self.expect_v_clear(0xE3);
        self.expect_z_set(0xE3);
        self.expect_n_clear(0xE3);
        self.expect_a_eq(0x00, 0xE3);
        self.mark_arithmetic_matrix_case(CPU_ARITHMETIC_MATRIX_ADC_MASK_ADDR, 0x02);
        ...
        self.expect_z_clear(0xE3);
        self.expect_n_clear(0xE3);
        self.expect_a_eq(0x03, 0xE3);
        self.mark_arithmetic_matrix_case(CPU_ARITHMETIC_MATRIX_ADC_MASK_ADDR, 0x04);

        self.asm.clc();
        self.asm.clv();
        self.asm.lda_imm(0x80);
        self.asm.adc_imm(0x80);
        self.asm
            .sta_abs(CPU_ARITHMETIC_MATRIX_ADC_NEG_OVERFLOW_RESULT_ADDR);
        self.expect_c_set(0xE3);
        self.expect_v_set(0xE3);
        ...
        self.expect_n_clear(0xE3);
        self.expect_a_eq(0x00, 0xE3);
        self.mark_arithmetic_matrix_case(CPU_ARITHMETIC_MATRIX_ADC_MASK_ADDR, 0x08);

        self.asm.sec();
        self.asm.clv();
        self.asm.lda_imm(0x03);
        self.asm.sbc_imm(0x01);
        self.asm
            .sta_abs(CPU_ARITHMETIC_MATRIX_SBC_NO_BORROW_RESULT_ADDR);
        self.expect_c_set(0xE4);
        self.expect_v_clear(0xE4);
        self.expect_z_clear(0xE4);
        self.expect_n_clear(0xE4);
        self.expect_a_eq(0x02, 0xE4);
        self.mark_arithmetic_matrix_case(CPU_ARITHMETIC_MATRIX_SBC_MASK_ADDR, 0x01);
        ...
        self.asm
            .sta_abs(CPU_ARITHMETIC_MATRIX_SBC_BORROW_RESULT_ADDR);
        self.expect_c_clear(0xE4);
        self.expect_v_clear(0xE4);
        self.expect_z_clear(0xE4);
        self.expect_n_set(0xE4);
        self.expect_a_eq(0xFF, 0xE4);
        self.mark_arithmetic_matrix_case(CPU_ARITHMETIC_MATRIX_SBC_MASK_ADDR, 0x02);
        ...
        self.asm
            .sta_abs(CPU_ARITHMETIC_MATRIX_SBC_OVERFLOW_RESULT_ADDR);
        self.expect_c_set(0xE4);
        self.expect_v_set(0xE4);
        self.expect_z_clear(0xE4);
        self.expect_n_clear(0xE4);
        self.expect_a_eq(0x7F, 0xE4);
        self.mark_arithmetic_matrix_case(CPU_ARITHMETIC_MATRIX_SBC_MASK_ADDR, 0x04);

        self.asm.clc();
        self.asm.clv();
        self.asm.lda_imm(0x01);
        self.asm.sbc_imm(0x00);
        self.asm
            .sta_abs(CPU_ARITHMETIC_MATRIX_SBC_BORROW_IN_RESULT_ADDR);
        self.expect_c_set(0xE4);
        self.expect_v_clear(0xE4);
        self.expect_z_set(0xE4);
        self.expect_n_clear(0xE4);
        self.expect_a_eq(0x00, 0xE4);
        self.mark_arithmetic_matrix_case(CPU_ARITHMETIC_MATRIX_SBC_MASK_ADDR, 0x08);

        self.asm
            .label(CPU_ARITHMETIC_MATRIX_SUMMARY_LABEL)
            .expect("diagnostic arithmetic summary label should not collide");
        self.asm.lda_abs(CPU_ARITHMETIC_MATRIX_ADC_MASK_ADDR);
        self.expect_a_eq(CPU_ARITHMETIC_MATRIX_EXPECTED_ADC_MASK, 0xE5);
        self.asm.lda_abs(CPU_ARITHMETIC_MATRIX_SBC_MASK_ADDR);
        self.expect_a_eq(CPU_ARITHMETIC_MATRIX_EXPECTED_SBC_MASK, 0xE5);
...
impl Assembler {
    ...
    fn lda_indexed_indirect(&mut self, addr: u8) {
        self.op_zp(0xA1, addr);
    }
    ...
    fn lda_abs_y(&mut self, addr: u16) {
        self.op_abs(0xB9, addr);
    }
    ...
    fn ldx_zp(&mut self, addr: u8) {
        self.op_zp(0xA6, addr);
    }
    ...
    fn ldy_zp_x(&mut self, addr: u8) {
        self.op_zp(0xB4, addr);
    }

    fn ldy_abs_x(&mut self, addr: u16) {
        self.op_abs(0xBC, addr);
    }
    ...
    fn sta_indirect_y(&mut self, addr: u8) {
        self.op_zp(0x91, addr);
    }
    ...
    fn stx_zp_y(&mut self, addr: u8) {
        self.op_zp(0x96, addr);
    }

    fn sty_abs(&mut self, addr: u16) {
        self.op_abs(0x8C, addr);
    }

    fn sty_zp_x(&mut self, addr: u8) {
        self.op_zp(0x94, addr);
    }
    ...
    fn bvc(&mut self, label: &str) {
        self.op_rel(0x50, label);
    }

    fn bvs(&mut self, label: &str) {
        self.op_rel(0x70, label);
    }
...
fn cpu_load_store_matrix_telemetry(ram: &[u8]) -> CpuLoadStoreMatrixTelemetry {
    let load_mask = ram[(CPU_LOAD_STORE_MATRIX_LOAD_MASK_ADDR & 0x07FF) as usize];
    let store_mask = ram[(CPU_LOAD_STORE_MATRIX_STORE_MASK_ADDR & 0x07FF) as usize];
    let transfer_mask = ram[(CPU_LOAD_STORE_MATRIX_TRANSFER_MASK_ADDR & 0x07FF) as usize];
    let load_a_result = ram[(CPU_LOAD_STORE_MATRIX_LOAD_A_RESULT_ADDR & 0x07FF) as usize];
    let load_x_result = ram[(CPU_LOAD_STORE_MATRIX_LOAD_X_RESULT_ADDR & 0x07FF) as usize];
    ...
    let load_y_zp_x_result = ram[(CPU_LOAD_STORE_MATRIX_LOAD_Y_ZP_X_RESULT_ADDR & 0x07FF) as usize];
    let load_x_zp_result = ram[(CPU_LOAD_STORE_MATRIX_LOAD_X_ZP_RESULT_ADDR & 0x07FF) as usize];
    ...
    let transfer_tax_result =
        ram[(CPU_LOAD_STORE_MATRIX_TRANSFER_TAX_RESULT_ADDR & 0x07FF) as usize];
    let transfer_tay_result =
        ram[(CPU_LOAD_STORE_MATRIX_TRANSFER_TAY_RESULT_ADDR & 0x07FF) as usize];
    let transfer_txa_result =
        ram[(CPU_LOAD_STORE_MATRIX_TRANSFER_TXA_RESULT_ADDR & 0x07FF) as usize];
    let transfer_tya_result =
        ram[(CPU_LOAD_STORE_MATRIX_TRANSFER_TYA_RESULT_ADDR & 0x07FF) as usize];
    let observed_case_count = ram[(CPU_LOAD_STORE_MATRIX_CASE_COUNT_ADDR & 0x07FF) as usize];

    CpuLoadStoreMatrixTelemetry {
        expected_case_count: CPU_LOAD_STORE_MATRIX_EXPECTED_CASE_COUNT,
        observed_case_count,
        expected_load_mask: CPU_LOAD_STORE_MATRIX_EXPECTED_LOAD_MASK,
        expected_load_mask_hex: hex_byte(CPU_LOAD_STORE_MATRIX_EXPECTED_LOAD_MASK),
        load_mask,
        load_mask_hex: hex_byte(load_mask),
        expected_store_mask: CPU_LOAD_STORE_MATRIX_EXPECTED_STORE_MASK,
        expected_store_mask_hex: hex_byte(CPU_LOAD_STORE_MATRIX_EXPECTED_STORE_MASK),
        store_mask,
        store_mask_hex: hex_byte(store_mask),
        expected_transfer_mask: CPU_LOAD_STORE_MATRIX_EXPECTED_TRANSFER_MASK,
        expected_transfer_mask_hex: hex_byte(CPU_LOAD_STORE_MATRIX_EXPECTED_TRANSFER_MASK),
        transfer_mask,
        transfer_mask_hex: hex_byte(transfer_mask),
        load_a_result,
        load_a_result_hex: hex_byte(load_a_result),
        load_x_result,
        load_x_result_hex: hex_byte(load_x_result),
        load_y_result,
        ...
        load_y_zp_x_result,
        load_y_zp_x_result_hex: hex_byte(load_y_zp_x_result),
        load_x_zp_result,
        load_x_zp_result_hex: hex_byte(load_x_zp_result),
        store_a_result,
        store_a_result_hex: hex_byte(store_a_result),
        store_x_result,
        store_x_result_hex: hex_byte(store_x_result),
        store_y_result,
        store_y_result_hex: hex_byte(store_y_result),
        transfer_tax_result,
        transfer_tax_result_hex: hex_byte(transfer_tax_result),
        transfer_tay_result,
        transfer_tay_result_hex: hex_byte(transfer_tay_result),
        transfer_txa_result,
        transfer_txa_result_hex: hex_byte(transfer_txa_result),
        transfer_tya_result,
        transfer_tya_result_hex: hex_byte(transfer_tya_result),
        passed: observed_case_count == CPU_LOAD_STORE_MATRIX_EXPECTED_CASE_COUNT
            && load_mask == CPU_LOAD_STORE_MATRIX_EXPECTED_LOAD_MASK
            && store_mask == CPU_LOAD_STORE_MATRIX_EXPECTED_STORE_MASK
            && transfer_mask == CPU_LOAD_STORE_MATRIX_EXPECTED_TRANSFER_MASK
            && load_a_result == 0x5A
            && load_x_result == 0x80
            && load_y_result == 0x7E
            && load_indirect_y_result == 0x91
            && load_x_zp_y_result == 0x42
            && load_y_zp_x_result == 0x39
            && load_x_zp_result == 0x24
            && store_a_result == 0xC3
            && store_x_result == 0x5E
            && store_y_result == 0xA7
            && transfer_tax_result == 0x44
...
fn cpu_alu_index_matrix_telemetry(ram: &[u8]) -> CpuAluIndexMatrixTelemetry {
    ...
    let ora_zp_result = ram[(CPU_ALU_INDEX_MATRIX_ORA_ZP_RESULT_ADDR & 0x07FF) as usize];
    let eor_result = ram[(CPU_ALU_INDEX_MATRIX_EOR_RESULT_ADDR & 0x07FF) as usize];
    let eor_zp_result = ram[(CPU_ALU_INDEX_MATRIX_EOR_ZP_RESULT_ADDR & 0x07FF) as usize];
    ...
    CpuAluIndexMatrixTelemetry {
        ...
        ora_zp_result,
        ora_zp_result_hex: hex_byte(ora_zp_result),
        eor_result,
        eor_result_hex: hex_byte(eor_result),
        eor_zp_result,
        eor_zp_result_hex: hex_byte(eor_zp_result),
        inx_result,
...
fn cpu_arithmetic_matrix_telemetry(ram: &[u8]) -> CpuArithmeticMatrixTelemetry {
    let adc_mask = ram[(CPU_ARITHMETIC_MATRIX_ADC_MASK_ADDR & 0x07FF) as usize];
    let sbc_mask = ram[(CPU_ARITHMETIC_MATRIX_SBC_MASK_ADDR & 0x07FF) as usize];
    let adc_overflow_result =
        ram[(CPU_ARITHMETIC_MATRIX_ADC_OVERFLOW_RESULT_ADDR & 0x07FF) as usize];
    let adc_carry_zero_result =
        ram[(CPU_ARITHMETIC_MATRIX_ADC_CARRY_ZERO_RESULT_ADDR & 0x07FF) as usize];
    let adc_carry_in_result =
        ram[(CPU_ARITHMETIC_MATRIX_ADC_CARRY_IN_RESULT_ADDR & 0x07FF) as usize];
    let adc_negative_overflow_result =
        ram[(CPU_ARITHMETIC_MATRIX_ADC_NEG_OVERFLOW_RESULT_ADDR & 0x07FF) as usize];
    let sbc_no_borrow_result =
        ram[(CPU_ARITHMETIC_MATRIX_SBC_NO_BORROW_RESULT_ADDR & 0x07FF) as usize];
    let sbc_borrow_result = ram[(CPU_ARITHMETIC_MATRIX_SBC_BORROW_RESULT_ADDR & 0x07FF) as usize];
    let sbc_overflow_result =
        ram[(CPU_ARITHMETIC_MATRIX_SBC_OVERFLOW_RESULT_ADDR & 0x07FF) as usize];
    let sbc_borrow_in_result =
        ram[(CPU_ARITHMETIC_MATRIX_SBC_BORROW_IN_RESULT_ADDR & 0x07FF) as usize];
    let observed_case_count = ram[(CPU_ARITHMETIC_MATRIX_CASE_COUNT_ADDR & 0x07FF) as usize];

    CpuArithmeticMatrixTelemetry {
        expected_case_count: CPU_ARITHMETIC_MATRIX_EXPECTED_CASE_COUNT,
        observed_case_count,
        expected_adc_mask: CPU_ARITHMETIC_MATRIX_EXPECTED_ADC_MASK,
        expected_adc_mask_hex: hex_byte(CPU_ARITHMETIC_MATRIX_EXPECTED_ADC_MASK),
        adc_mask,
        adc_mask_hex: hex_byte(adc_mask),
        expected_sbc_mask: CPU_ARITHMETIC_MATRIX_EXPECTED_SBC_MASK,
        expected_sbc_mask_hex: hex_byte(CPU_ARITHMETIC_MATRIX_EXPECTED_SBC_MASK),
        sbc_mask,
        sbc_mask_hex: hex_byte(sbc_mask),
        adc_overflow_result,
        adc_overflow_result_hex: hex_byte(adc_overflow_result),
        adc_carry_zero_result,
        adc_carry_zero_result_hex: hex_byte(adc_carry_zero_result),
        adc_carry_in_result,
        adc_carry_in_result_hex: hex_byte(adc_carry_in_result),
        adc_negative_overflow_result,
        adc_negative_overflow_result_hex: hex_byte(adc_negative_overflow_result),
        sbc_no_borrow_result,
        sbc_no_borrow_result_hex: hex_byte(sbc_no_borrow_result),
        sbc_borrow_result,
        sbc_borrow_result_hex: hex_byte(sbc_borrow_result),
        sbc_overflow_result,
        sbc_overflow_result_hex: hex_byte(sbc_overflow_result),
        sbc_borrow_in_result,
        sbc_borrow_in_result_hex: hex_byte(sbc_borrow_in_result),
        passed: observed_case_count == CPU_ARITHMETIC_MATRIX_EXPECTED_CASE_COUNT
            && adc_mask == CPU_ARITHMETIC_MATRIX_EXPECTED_ADC_MASK
            && sbc_mask == CPU_ARITHMETIC_MATRIX_EXPECTED_SBC_MASK
            && adc_overflow_result == 0x80
            && adc_carry_zero_result == 0x00
            && adc_carry_in_result == 0x03
            && adc_negative_overflow_result == 0x00
            && sbc_no_borrow_result == 0x02
...
fn mapper4_mmc3_a12_gate_telemetry(
    observation: &Mapper4Mmc3A12GateObservation,
) -> Mapper4Mmc3A12GateTelemetry {
    Mapper4Mmc3A12GateTelemetry {
        mapper: MAPPER4_MAPPER,
        prg_16k_banks: MAPPER4_PRG_16K_BANKS,
        chr_8k_banks: MAPPER4_CHR_8K_BANKS,
        irq_latch: MAPPER4_A12_GATE_IRQ_LATCH,
        irq_latch_hex: hex_byte(MAPPER4_A12_GATE_IRQ_LATCH),
        low_pattern_ctrl: MAPPER4_A12_GATE_LOW_PATTERN_CTRL,
        low_pattern_ctrl_hex: hex_byte(MAPPER4_A12_GATE_LOW_PATTERN_CTRL),
        high_pattern_ctrl: MAPPER4_A12_GATE_HIGH_PATTERN_CTRL,
        high_pattern_ctrl_hex: hex_byte(MAPPER4_A12_GATE_HIGH_PATTERN_CTRL),
        background_mask: MAPPER4_A12_GATE_BACKGROUND_MASK,
        background_mask_hex: hex_byte(MAPPER4_A12_GATE_BACKGROUND_MASK),
        expected_low_pattern_irq_count: MAPPER4_A12_GATE_EXPECTED_LOW_IRQ_COUNT,
        observed_low_pattern_irq_count: observation.low_pattern_irq_count,
        expected_high_pattern_irq_count: MAPPER4_A12_GATE_EXPECTED_HIGH_IRQ_COUNT,
        observed_high_pattern_irq_count: observation.high_pattern_irq_count,
        expected_case_count: MAPPER4_A12_GATE_EXPECTED_CASE_COUNT,
        observed_case_count: observation.observed_case_count,
        cycles: observation.cycles,
        frames: observation.frames,
        passed: observation.passed,
...
#[derive(Debug, Clone)]
struct Mapper4Mmc3A12GateObservation {
    low_pattern_irq_count: u8,
    high_pattern_irq_count: u8,
    observed_case_count: u8,
    cycles: u64,
    frames: u64,
    passed: bool,
...
impl Mapper4Mmc3A12GateObservation {
    fn failed(message: impl Into<String>) -> Self {
        Self {
...
fn run_mapper4_mmc3_a12_gate_variant() -> Mapper4Mmc3A12GateObservation {
    match try_run_mapper4_mmc3_a12_gate_variant() {
        Ok(observation) => observation,
        Err(error) => Mapper4Mmc3A12GateObservation::failed(error),
    }
}

fn try_run_mapper4_mmc3_a12_gate_variant() -> Result<Mapper4Mmc3A12GateObservation, String> {
    let rom = build_mapper4_mmc3_a12_gate_variant_cartridge()?;
    let cartridge = Cartridge::new(&rom)?;
    let mut bus = Bus::new(cartridge);
    let mut cpu = Cpu::new();
    cpu.reset(&mut bus);
    ...
    while cycles < cycle_limit {
        ...
        let status = read_ram_byte(&mut bus, STATUS_ADDR);
        if matches!(status, STATUS_PASS | STATUS_FAIL) {
            let low_pattern_irq_count = bus.cpu_read(MAPPER4_MMC3_A12_GATE_LOW_IRQ_COUNT_ADDR);
            let high_pattern_irq_count = bus.cpu_read(MAPPER4_MMC3_A12_GATE_HIGH_IRQ_COUNT_ADDR);
            let observed_case_count = bus.cpu_read(MAPPER4_MMC3_A12_GATE_CASE_COUNT_ADDR);
            let failure_code = read_ram_byte(&mut bus, FAILURE_CODE_ADDR);
            let passed = status == STATUS_PASS
                && low_pattern_irq_count == MAPPER4_A12_GATE_EXPECTED_LOW_IRQ_COUNT
                && high_pattern_irq_count == MAPPER4_A12_GATE_EXPECTED_HIGH_IRQ_COUNT
                && observed_case_count == MAPPER4_A12_GATE_EXPECTED_CASE_COUNT;
            let error = if passed {
                None
            } else if status == STATUS_FAIL {
                Some(format!(
                    "Mapper 4 MMC3 A12 gate variant reported FAIL with failure code 0x{failure_code:02X}"
                ))
            } else {
                Some(format!(
                    "Mapper 4 MMC3 A12 gate variant reached PASS with mismatched host observations: low IRQ {}, high IRQ {}, cases {}/{}",
                    low_pattern_irq_count,
                    high_pattern_irq_count,
                    observed_case_count,
                    MAPPER4_A12_GATE_EXPECTED_CASE_COUNT
                ))
            };
            return Ok(Mapper4Mmc3A12GateObservation {
                low_pattern_irq_count,
                high_pattern_irq_count,
                observed_case_count,
                cycles,
    ...
    Ok(Mapper4Mmc3A12GateObservation {
        low_pattern_irq_count: bus.cpu_read(MAPPER4_MMC3_A12_GATE_LOW_IRQ_COUNT_ADDR),
        high_pattern_irq_count: bus.cpu_read(MAPPER4_MMC3_A12_GATE_HIGH_IRQ_COUNT_ADDR),
        observed_case_count: bus.cpu_read(MAPPER4_MMC3_A12_GATE_CASE_COUNT_ADDR),
        cycles,
        frames,
        passed: false,
        error: Some(format!(
...
fn compare_observation_checksums(
    ...
    for path in [
        ...
        &["cpu_load_store_matrix", "load_mask"][..],
        &["cpu_load_store_matrix", "store_mask"][..],
        &["cpu_load_store_matrix", "transfer_mask"][..],
        &["cpu_load_store_matrix", "load_a_result"][..],
        ...
        &["cpu_load_store_matrix", "load_y_zp_x_result"][..],
        &["cpu_load_store_matrix", "load_x_zp_result"][..],
        &["cpu_load_store_matrix", "store_a_result"][..],
        &["cpu_load_store_matrix", "store_x_result"][..],
        &["cpu_load_store_matrix", "store_y_result"][..],
        &["cpu_load_store_matrix", "transfer_tax_result"][..],
        &["cpu_load_store_matrix", "transfer_tay_result"][..],
        &["cpu_load_store_matrix", "transfer_txa_result"][..],
        ...
        &["cpu_alu_index_matrix", "ora_zp_result"][..],
        &["cpu_alu_index_matrix", "eor_result"][..],
        &["cpu_alu_index_matrix", "eor_zp_result"][..],
        ...
        &["cpu_arithmetic_matrix", "adc_mask"][..],
        &["cpu_arithmetic_matrix", "sbc_mask"][..],
        &["cpu_arithmetic_matrix", "adc_overflow_result"][..],
        &["cpu_arithmetic_matrix", "adc_carry_zero_result"][..],
        &["cpu_arithmetic_matrix", "adc_carry_in_result"][..],
        &["cpu_arithmetic_matrix", "adc_negative_overflow_result"][..],
        &["cpu_arithmetic_matrix", "sbc_no_borrow_result"][..],
        &["cpu_arithmetic_matrix", "sbc_borrow_result"][..],
        &["cpu_arithmetic_matrix", "sbc_overflow_result"][..],
    ...
    fn headless_diagnostic_passes_and_collects_telemetry() {
        ...
        assert!(telemetry.analysis.coverage_gaps.iter().any(|gap| {
            gap.id == "input_port_matrix"
                ...
                && gap.current_coverage.contains("host input-remapping")
                && gap
                    .current_coverage
                ...
                && gap
                    .current_coverage
                    .contains("gilrs polling helper fixture")
                && gap
                    .missing_coverage
                    .contains("Live minifb Window key polling")
                && gap
                    .missing_coverage
                    .contains("physical or virtual gilrs device")
                && !gap.missing_coverage.contains("Disconnected-controller")
                && !gap.missing_coverage.contains("synthetic host snapshots")
                && !gap.missing_coverage.contains("typed adapter fixtures")
                && !gap
                    .suggested_next_test
                ...
                && gap
                    .suggested_next_test
                    .contains("local live-input polling harness")
        }));
