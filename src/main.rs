...
fn blend_bilinear_rgb(p00: u32, p10: u32, p01: u32, p11: u32, frac_x: u32, frac_y: u32) -> (u32, u32, u32) {
    ...
    // SWAR: process R+B channels together (16 bits apart, no overlap risk)
    ...
    // G channel separately (2-step blend: 6 multiplies instead of 8)
    let g00 = (p00 >> 8) & 0xFF;
    let g10 = (p10 >> 8) & 0xFF;
    let g01 = (p01 >> 8) & 0xFF;
    let g11 = (p11 >> 8) & 0xFF;
...
fn apply_phosphor_bloom(buffer: &mut [u32], width: usize, height: usize, bloom_strength: u32) {
    ...
    for y in 0..height {
        ...
        // Backward pass (right→left) — same row, still in L1 cache
        carry_r = 0;
        carry_g = 0;
        carry_b = 0;
...
fn crt_filter(input: &[u32], output: &mut Vec<u32>, sv_table: &[u8], distortion_table: &[(u32, u32)], crt_cfg: &CrtConfig, mask_table: &[(u16, u16, u16)], brightness: i32, contrast: i32) {
    ...
    // Pre-compute phosphor warmth LUT: brightness → (pr, pg, pb) multiplier
...
fn apply_glass_effects(
    ...
    if do_ghost {
        glass_inner_loop(buffer, ghost_source, glare_table, thickness_table, ghost_alpha_table,
            window_width, intensity_factor, tint_strength, true,
            corner_x_max, corner_y_max, ghost_shift_x, ghost_shift_y, ghost_h, ghost_w, ghost_stride);
    } else {
        glass_inner_loop(buffer, ghost_source, glare_table, thickness_table, ghost_alpha_table,
            window_width, intensity_factor, tint_strength, false,
            corner_x_max, corner_y_max, ghost_shift_x, ghost_shift_y, ghost_h, ghost_w, ghost_stride);
    }
}
...
fn glass_inner_loop(
    ...
    for y in 0..SCREEN_H {
        ...
        for x in 0..SCREEN_W {
            ...
            // Compute brightness once for both tint and glare
