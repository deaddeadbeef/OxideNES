...
fn generated_diagnostic_cartridge_runs_headlessly_to_pass() {
    ...
    assert!(telemetry
        ...
        .subsystem_summary
        .iter()
        .any(|entry| entry.subsystem == DiagnosticSubsystem::Ppu && entry.total == 6));
    assert!(telemetry
        .analysis
        .coverage
        .subsystem_summary
...
fn generated_diagnostic_cartridge_localizes_intentional_joypad_failure() {
    ...
    assert_eq!(telemetry.analysis.timing.not_started_tests, 17);
...
fn generated_diagnostic_cartridge_localizes_timeout() {
    ...
    assert!(report.contains("| Not started tests | 24 |"));
