...
fn diagnostic_cli_writes_exhaustive_input_sweep_artifacts() {
    ...
    assert_eq!(sweep["telemetry_schema_version"], Value::from(76));
...
fn diagnostic_cli_writes_ai_ready_scenario_suite() {
    ...
    assert_eq!(manifest["telemetry_schema_version"], Value::from(76));
    ...
    assert_eq!(manifest["scenario_count"], Value::from(45));
    assert_eq!(
    ...
    assert_eq!(manifest["analysis"]["scenario_count"], Value::from(45));
    assert_eq!(
    ...
    assert!(manifest["analysis"]["known_divergence_scenario_ids"]
        ...
        .expect("known divergence scenario ids should be an array")
        .iter()
        .any(|id| id == &Value::String("cpu_branch_matrix_fault".to_string())));
    assert!(manifest["analysis"]["known_divergence_scenario_ids"]
        .as_array()
        .expect("known divergence scenario ids should be an array")
        .iter()
        .any(|id| id == &Value::String("cpu_stack_matrix_fault".to_string())));
    assert!(manifest["analysis"]["known_divergence_scenario_ids"]
        .as_array()
        .expect("known divergence scenario ids should be an array")
        .iter()
        .any(|id| id == &Value::String("cpu_interrupt_matrix_fault".to_string())));
    assert!(manifest["analysis"]["known_divergence_scenario_ids"]
        .as_array()
        .expect("known divergence scenario ids should be an array")
        .iter()
        .any(|id| id == &Value::String("cpu_accumulator_matrix_fault".to_string())));
    assert!(manifest["analysis"]["known_divergence_scenario_ids"]
        .as_array()
        .expect("known divergence scenario ids should be an array")
        .iter()
        .any(|id| id == &Value::String("cpu_compare_matrix_fault".to_string())));
    assert!(manifest["analysis"]["known_divergence_scenario_ids"]
        .as_array()
        .expect("known divergence scenario ids should be an array")
        .iter()
        .any(|id| id == &Value::String("cpu_load_store_matrix_fault".to_string())));
    assert!(manifest["analysis"]["known_divergence_scenario_ids"]
        .as_array()
        .expect("known divergence scenario ids should be an array")
        .iter()
        .any(|id| id == &Value::String("cpu_alu_index_matrix_fault".to_string())));
    assert!(manifest["analysis"]["known_divergence_scenario_ids"]
        .as_array()
        .expect("known divergence scenario ids should be an array")
        .iter()
        .any(|id| id == &Value::String("cpu_arithmetic_matrix_fault".to_string())));
    assert!(manifest["analysis"]["known_divergence_scenario_ids"]
        .as_array()
        .expect("known divergence scenario ids should be an array")
        .iter()
        .any(|id| id == &Value::String("cpu_status_matrix_fault".to_string())));
    assert!(manifest["analysis"]["known_divergence_scenario_ids"]
        .as_array()
        .expect("known divergence scenario ids should be an array")
        .iter()
    ...
    assert!(manifest["analysis"]["known_divergence_scenario_ids"]
        ...
        .expect("known divergence scenario ids should be an array")
        .iter()
        .any(|id| id == &Value::String("ppu_pixel_phase_fault".to_string())));
    assert!(manifest["analysis"]["known_divergence_scenario_ids"]
        .as_array()
        .expect("known divergence scenario ids should be an array")
        .iter()
        .any(|id| id == &Value::String("ppu_attribute_quadrant_fault".to_string())));
    let attention_queue = manifest["analysis"]["attention_queue"]
        .as_array()
        .expect("attention queue should be an array");
    assert_eq!(attention_queue.len(), 37);
    ...
    let ppu_pixel_attention = find_attention_item(attention_queue, "ppu_pixel_phase_fault");
    assert_eq!(
        ppu_pixel_attention["priority"],
        Value::String("known_divergence".to_string())
    );
    assert_eq!(
        ppu_pixel_attention["focus_domain"],
        Value::String("ppu.pixel_phase".to_string())
    );
    assert_eq!(
        ppu_pixel_attention["actual_health"],
        Value::String("host_validation_failed".to_string())
    );
    assert_eq!(
        ppu_pixel_attention["next_artifact"],
        Value::String("ppu_pixel_phase_fault/comparison.json".to_string())
    );
    let ppu_attribute_attention =
        find_attention_item(attention_queue, "ppu_attribute_quadrant_fault");
    assert_eq!(
        ppu_attribute_attention["priority"],
        Value::String("known_divergence".to_string())
    );
    assert_eq!(
        ppu_attribute_attention["focus_domain"],
        Value::String("ppu.attribute_quadrant".to_string())
    );
    assert_eq!(
        ppu_attribute_attention["actual_health"],
        Value::String("host_validation_failed".to_string())
    );
    assert_eq!(
        ppu_attribute_attention["next_artifact"],
        Value::String("ppu_attribute_quadrant_fault/comparison.json".to_string())
    );
    ...
    let branch_attention = find_attention_item(attention_queue, "cpu_branch_matrix_fault");
    assert_eq!(
        branch_attention["priority"],
        Value::String("known_divergence".to_string())
    );
    assert_eq!(
        branch_attention["focus_domain"],
        Value::String("cpu.branch.condition_matrix".to_string())
    );
    assert_eq!(
        branch_attention["next_artifact"],
        Value::String("cpu_branch_matrix_fault/comparison.json".to_string())
    );
    let stack_attention = find_attention_item(attention_queue, "cpu_stack_matrix_fault");
    assert_eq!(
        stack_attention["priority"],
        Value::String("known_divergence".to_string())
    );
    assert_eq!(
        stack_attention["focus_domain"],
        Value::String("cpu.stack.status_matrix".to_string())
    );
    assert_eq!(
        stack_attention["next_artifact"],
        Value::String("cpu_stack_matrix_fault/comparison.json".to_string())
    );
    let interrupt_attention = find_attention_item(attention_queue, "cpu_interrupt_matrix_fault");
    assert_eq!(
        interrupt_attention["priority"],
        Value::String("known_divergence".to_string())
    );
    assert_eq!(
        interrupt_attention["focus_domain"],
        Value::String("cpu.interrupt.brk_rti_matrix".to_string())
    );
    assert_eq!(
        interrupt_attention["next_artifact"],
        Value::String("cpu_interrupt_matrix_fault/comparison.json".to_string())
    );
    let accumulator_attention =
        find_attention_item(attention_queue, "cpu_accumulator_matrix_fault");
    assert_eq!(
        accumulator_attention["priority"],
        Value::String("known_divergence".to_string())
    );
    assert_eq!(
        accumulator_attention["focus_domain"],
        Value::String("cpu.accumulator.shift_rotate".to_string())
    );
    assert_eq!(
        accumulator_attention["next_artifact"],
        Value::String("cpu_accumulator_matrix_fault/comparison.json".to_string())
    );
    let compare_attention = find_attention_item(attention_queue, "cpu_compare_matrix_fault");
    assert_eq!(
        compare_attention["priority"],
        Value::String("known_divergence".to_string())
    );
    assert_eq!(
        compare_attention["focus_domain"],
        Value::String("cpu.compare.flags".to_string())
    );
    assert_eq!(
        compare_attention["next_artifact"],
        Value::String("cpu_compare_matrix_fault/comparison.json".to_string())
    );
    let load_store_attention = find_attention_item(attention_queue, "cpu_load_store_matrix_fault");
    assert_eq!(
        load_store_attention["priority"],
        Value::String("known_divergence".to_string())
    );
    assert_eq!(
        load_store_attention["focus_domain"],
        Value::String("cpu.load_store.transfer_matrix".to_string())
    );
    assert_eq!(
        load_store_attention["next_artifact"],
        Value::String("cpu_load_store_matrix_fault/comparison.json".to_string())
    );
    let alu_index_attention = find_attention_item(attention_queue, "cpu_alu_index_matrix_fault");
    assert_eq!(
        alu_index_attention["priority"],
        Value::String("known_divergence".to_string())
    );
    assert_eq!(
        alu_index_attention["focus_domain"],
        Value::String("cpu.alu_index.logic_flags".to_string())
    );
    assert_eq!(
        alu_index_attention["next_artifact"],
        Value::String("cpu_alu_index_matrix_fault/comparison.json".to_string())
    );
    let arithmetic_attention = find_attention_item(attention_queue, "cpu_arithmetic_matrix_fault");
    assert_eq!(
        arithmetic_attention["priority"],
        Value::String("known_divergence".to_string())
    );
    assert_eq!(
        arithmetic_attention["focus_domain"],
        Value::String("cpu.arithmetic.adc_sbc_flags".to_string())
    );
    assert_eq!(
        arithmetic_attention["next_artifact"],
        Value::String("cpu_arithmetic_matrix_fault/comparison.json".to_string())
    );
    let status_attention = find_attention_item(attention_queue, "cpu_status_matrix_fault");
    assert_eq!(
        status_attention["priority"],
        Value::String("known_divergence".to_string())
    );
    assert_eq!(
        status_attention["focus_domain"],
        Value::String("cpu.status.bit_flags".to_string())
    );
    assert_eq!(
        status_attention["next_artifact"],
        Value::String("cpu_status_matrix_fault/comparison.json".to_string())
    );
    ...
    assert_eq!(observer["telemetry_schema_version"], Value::from(76));
    ...
    let ppu_pixel_action = find_observer_action(observer_actions, "ppu_pixel_phase_fault");
    assert_eq!(
        ppu_pixel_action["primary_artifact"],
        Value::String("ppu_pixel_phase_fault/comparison.json".to_string())
    );
    assert!(ppu_pixel_action["evidence"]
        .as_array()
        .expect("PPU pixel-phase action evidence should be an array")
        .iter()
        .any(|entry| entry == &Value::String("focus_domain=ppu.pixel_phase".to_string())));
    assert!(ppu_pixel_action["evidence"]
        .as_array()
        .expect("PPU pixel-phase action evidence should be an array")
        .iter()
        .any(|entry| entry
            == &Value::String("failed_probe_ids=ppu.pixel_phase.signature".to_string())));
    let ppu_attribute_action =
        find_observer_action(observer_actions, "ppu_attribute_quadrant_fault");
    assert_eq!(
        ppu_attribute_action["primary_artifact"],
        Value::String("ppu_attribute_quadrant_fault/comparison.json".to_string())
    );
    assert!(ppu_attribute_action["evidence"]
        .as_array()
        .expect("PPU attribute-quadrant action evidence should be an array")
        .iter()
        .any(|entry| entry == &Value::String("focus_domain=ppu.attribute_quadrant".to_string())));
    assert!(ppu_attribute_action["evidence"]
        .as_array()
        .expect("PPU attribute-quadrant action evidence should be an array")
        .iter()
        .any(|entry| entry
            == &Value::String("failed_probe_ids=ppu.attribute_quadrant.signature".to_string())));
    ...
    let branch_action = find_observer_action(observer_actions, "cpu_branch_matrix_fault");
    assert_eq!(
        branch_action["primary_artifact"],
        Value::String("cpu_branch_matrix_fault/comparison.json".to_string())
    );
    assert!(branch_action["evidence"]
        .as_array()
        .expect("CPU branch action evidence should be an array")
        .iter()
        .any(
            |entry| entry == &Value::String("focus_domain=cpu.branch.condition_matrix".to_string())
        ));
    assert!(branch_action["evidence"]
        .as_array()
        .expect("CPU branch action evidence should be an array")
        ...
        .any(|entry| entry
            == &Value::String(
                "failed_probe_ids=cartridge.status.pass,cartridge.test.39.result".to_string()
            )));
    let stack_action = find_observer_action(observer_actions, "cpu_stack_matrix_fault");
    assert_eq!(
        stack_action["primary_artifact"],
        Value::String("cpu_stack_matrix_fault/comparison.json".to_string())
    );
    assert!(stack_action["evidence"]
        .as_array()
        .expect("CPU stack action evidence should be an array")
        .iter()
        .any(|entry| entry == &Value::String("focus_domain=cpu.stack.status_matrix".to_string())));
    assert!(stack_action["evidence"]
        .as_array()
        .expect("CPU stack action evidence should be an array")
        .iter()
        .any(|entry| entry
            == &Value::String(
                "failed_probe_ids=cartridge.status.pass,cartridge.test.40.result".to_string()
            )));
    let interrupt_action = find_observer_action(observer_actions, "cpu_interrupt_matrix_fault");
    assert_eq!(
        interrupt_action["primary_artifact"],
        Value::String("cpu_interrupt_matrix_fault/comparison.json".to_string())
    );
    assert!(interrupt_action["evidence"]
        .as_array()
        .expect("CPU interrupt action evidence should be an array")
        .iter()
        .any(|entry| entry
            == &Value::String("focus_domain=cpu.interrupt.brk_rti_matrix".to_string())));
    assert!(interrupt_action["evidence"]
        .as_array()
        .expect("CPU interrupt action evidence should be an array")
        .iter()
        .any(|entry| entry
            == &Value::String(
                "failed_probe_ids=cartridge.status.pass,cartridge.test.41.result".to_string()
            )));
    let accumulator_action = find_observer_action(observer_actions, "cpu_accumulator_matrix_fault");
    assert_eq!(
        accumulator_action["primary_artifact"],
        Value::String("cpu_accumulator_matrix_fault/comparison.json".to_string())
    );
    assert!(accumulator_action["evidence"]
        .as_array()
        .expect("CPU accumulator action evidence should be an array")
        .iter()
        .any(|entry| entry
            == &Value::String("focus_domain=cpu.accumulator.shift_rotate".to_string())));
    assert!(accumulator_action["evidence"]
        .as_array()
        .expect("CPU accumulator action evidence should be an array")
        .iter()
        .any(|entry| entry
            == &Value::String(
                "failed_probe_ids=cartridge.status.pass,cartridge.test.42.result".to_string()
            )));
    let compare_action = find_observer_action(observer_actions, "cpu_compare_matrix_fault");
    assert_eq!(
        compare_action["primary_artifact"],
        Value::String("cpu_compare_matrix_fault/comparison.json".to_string())
    );
    assert!(compare_action["evidence"]
        .as_array()
        .expect("CPU compare action evidence should be an array")
        .iter()
        .any(|entry| entry == &Value::String("focus_domain=cpu.compare.flags".to_string())));
    assert!(compare_action["evidence"]
        .as_array()
        .expect("CPU compare action evidence should be an array")
        .iter()
        .any(|entry| entry
            == &Value::String(
                "failed_probe_ids=cartridge.status.pass,cartridge.test.43.result".to_string()
            )));
    let load_store_action = find_observer_action(observer_actions, "cpu_load_store_matrix_fault");
    assert_eq!(
        load_store_action["primary_artifact"],
        Value::String("cpu_load_store_matrix_fault/comparison.json".to_string())
    );
    assert!(load_store_action["evidence"]
        .as_array()
        .expect("CPU load/store action evidence should be an array")
        .iter()
        .any(|entry| entry
            == &Value::String("focus_domain=cpu.load_store.transfer_matrix".to_string())));
    assert!(load_store_action["evidence"]
        .as_array()
        .expect("CPU load/store action evidence should be an array")
        .iter()
        .any(|entry| entry
            == &Value::String(
                "failed_probe_ids=cartridge.status.pass,cartridge.test.44.result".to_string()
            )));
    let alu_index_action = find_observer_action(observer_actions, "cpu_alu_index_matrix_fault");
    assert_eq!(
        alu_index_action["primary_artifact"],
        Value::String("cpu_alu_index_matrix_fault/comparison.json".to_string())
    );
    assert!(
        alu_index_action["evidence"]
            .as_array()
            .expect("CPU ALU/index action evidence should be an array")
            .iter()
            .any(|entry| entry
                == &Value::String("focus_domain=cpu.alu_index.logic_flags".to_string()))
    );
    assert!(alu_index_action["evidence"]
        .as_array()
        .expect("CPU ALU/index action evidence should be an array")
        .iter()
        .any(|entry| entry
            == &Value::String(
                "failed_probe_ids=cartridge.status.pass,cartridge.test.45.result".to_string()
            )));
    let arithmetic_action = find_observer_action(observer_actions, "cpu_arithmetic_matrix_fault");
    assert_eq!(
        arithmetic_action["primary_artifact"],
        Value::String("cpu_arithmetic_matrix_fault/comparison.json".to_string())
    );
    assert!(arithmetic_action["evidence"]
        .as_array()
        .expect("CPU arithmetic action evidence should be an array")
        .iter()
        .any(|entry| entry
            == &Value::String("focus_domain=cpu.arithmetic.adc_sbc_flags".to_string())));
    assert!(arithmetic_action["evidence"]
        .as_array()
        .expect("CPU arithmetic action evidence should be an array")
        .iter()
        .any(|entry| entry
            == &Value::String(
                "failed_probe_ids=cartridge.status.pass,cartridge.test.46.result".to_string()
            )));
    let status_action = find_observer_action(observer_actions, "cpu_status_matrix_fault");
    assert_eq!(
        status_action["primary_artifact"],
        Value::String("cpu_status_matrix_fault/comparison.json".to_string())
    );
    assert!(status_action["evidence"]
        .as_array()
        .expect("CPU status/BIT action evidence should be an array")
        .iter()
        .any(|entry| entry == &Value::String("focus_domain=cpu.status.bit_flags".to_string())));
    assert!(status_action["evidence"]
        .as_array()
        .expect("CPU status/BIT action evidence should be an array")
        .iter()
        .any(|entry| entry
            == &Value::String(
                "failed_probe_ids=cartridge.status.pass,cartridge.test.47.result".to_string()
            )));
    ...
    let branch_observation = find_observer_observation(observations, "cpu_branch_matrix_fault");
    assert_eq!(
        branch_observation["role"],
        Value::String("expected_failure_fixture".to_string())
    );
    assert_eq!(
        branch_observation["outcome"],
        Value::String("expected_baseline_divergence".to_string())
    );
    assert_eq!(
        branch_observation["health"],
        Value::String("cartridge_assertion_failed".to_string())
    );
    assert_eq!(
        branch_observation["focus_domain"],
        Value::String("cpu.branch.condition_matrix".to_string())
    );
    assert_eq!(
        branch_observation["next_artifact"],
        Value::String("cpu_branch_matrix_fault/comparison.json".to_string())
    );
    let stack_observation = find_observer_observation(observations, "cpu_stack_matrix_fault");
    assert_eq!(
        stack_observation["role"],
        Value::String("expected_failure_fixture".to_string())
    );
    assert_eq!(
        stack_observation["outcome"],
        Value::String("expected_baseline_divergence".to_string())
    );
    assert_eq!(
        stack_observation["health"],
        Value::String("cartridge_assertion_failed".to_string())
    );
    assert_eq!(
        stack_observation["focus_domain"],
        Value::String("cpu.stack.status_matrix".to_string())
    );
    assert_eq!(
        stack_observation["next_artifact"],
        Value::String("cpu_stack_matrix_fault/comparison.json".to_string())
    );
    let interrupt_observation =
        find_observer_observation(observations, "cpu_interrupt_matrix_fault");
    assert_eq!(
        interrupt_observation["role"],
        Value::String("expected_failure_fixture".to_string())
    );
    assert_eq!(
        interrupt_observation["outcome"],
        Value::String("expected_baseline_divergence".to_string())
    );
    assert_eq!(
        interrupt_observation["health"],
        Value::String("cartridge_assertion_failed".to_string())
    );
    assert_eq!(
        interrupt_observation["focus_domain"],
        Value::String("cpu.interrupt.brk_rti_matrix".to_string())
    );
    assert_eq!(
        interrupt_observation["next_artifact"],
        Value::String("cpu_interrupt_matrix_fault/comparison.json".to_string())
    );
    let accumulator_observation =
        find_observer_observation(observations, "cpu_accumulator_matrix_fault");
    assert_eq!(
        accumulator_observation["role"],
        Value::String("expected_failure_fixture".to_string())
    );
    assert_eq!(
        accumulator_observation["outcome"],
        Value::String("expected_baseline_divergence".to_string())
    );
    assert_eq!(
        accumulator_observation["health"],
        Value::String("cartridge_assertion_failed".to_string())
    );
    assert_eq!(
        accumulator_observation["focus_domain"],
        Value::String("cpu.accumulator.shift_rotate".to_string())
    );
    assert_eq!(
        accumulator_observation["next_artifact"],
        Value::String("cpu_accumulator_matrix_fault/comparison.json".to_string())
    );
    let compare_observation = find_observer_observation(observations, "cpu_compare_matrix_fault");
    assert_eq!(
        compare_observation["role"],
        Value::String("expected_failure_fixture".to_string())
    );
    assert_eq!(
        compare_observation["outcome"],
        Value::String("expected_baseline_divergence".to_string())
    );
    assert_eq!(
        compare_observation["health"],
        Value::String("cartridge_assertion_failed".to_string())
    );
    assert_eq!(
        compare_observation["focus_domain"],
        Value::String("cpu.compare.flags".to_string())
    );
    assert_eq!(
        compare_observation["next_artifact"],
        Value::String("cpu_compare_matrix_fault/comparison.json".to_string())
    );
    let load_store_observation =
        find_observer_observation(observations, "cpu_load_store_matrix_fault");
    assert_eq!(
        load_store_observation["role"],
        Value::String("expected_failure_fixture".to_string())
    );
    assert_eq!(
        load_store_observation["outcome"],
        Value::String("expected_baseline_divergence".to_string())
    );
    assert_eq!(
        load_store_observation["health"],
        Value::String("cartridge_assertion_failed".to_string())
    );
    assert_eq!(
        load_store_observation["focus_domain"],
        Value::String("cpu.load_store.transfer_matrix".to_string())
    );
    assert_eq!(
        load_store_observation["next_artifact"],
        Value::String("cpu_load_store_matrix_fault/comparison.json".to_string())
    );
    let alu_index_observation =
        find_observer_observation(observations, "cpu_alu_index_matrix_fault");
    assert_eq!(
        alu_index_observation["role"],
        Value::String("expected_failure_fixture".to_string())
    );
    assert_eq!(
        alu_index_observation["outcome"],
        Value::String("expected_baseline_divergence".to_string())
    );
    assert_eq!(
        alu_index_observation["health"],
        Value::String("cartridge_assertion_failed".to_string())
    );
    assert_eq!(
        alu_index_observation["focus_domain"],
        Value::String("cpu.alu_index.logic_flags".to_string())
    );
    assert_eq!(
        alu_index_observation["next_artifact"],
        Value::String("cpu_alu_index_matrix_fault/comparison.json".to_string())
    );
    let arithmetic_observation =
        find_observer_observation(observations, "cpu_arithmetic_matrix_fault");
    assert_eq!(
        arithmetic_observation["role"],
        Value::String("expected_failure_fixture".to_string())
    );
    assert_eq!(
        arithmetic_observation["outcome"],
        Value::String("expected_baseline_divergence".to_string())
    );
    assert_eq!(
        arithmetic_observation["health"],
        Value::String("cartridge_assertion_failed".to_string())
    );
    assert_eq!(
        arithmetic_observation["focus_domain"],
        Value::String("cpu.arithmetic.adc_sbc_flags".to_string())
    );
    assert_eq!(
        arithmetic_observation["next_artifact"],
        Value::String("cpu_arithmetic_matrix_fault/comparison.json".to_string())
    );
    let status_observation = find_observer_observation(observations, "cpu_status_matrix_fault");
    assert_eq!(
        status_observation["role"],
        Value::String("expected_failure_fixture".to_string())
    );
    assert_eq!(
        status_observation["outcome"],
        Value::String("expected_baseline_divergence".to_string())
    );
    assert_eq!(
        status_observation["health"],
        Value::String("cartridge_assertion_failed".to_string())
    );
    assert_eq!(
        status_observation["focus_domain"],
        Value::String("cpu.status.bit_flags".to_string())
    );
    assert_eq!(
        status_observation["next_artifact"],
        Value::String("cpu_status_matrix_fault/comparison.json".to_string())
    );
    ...
    let ppu_pixel_observation = find_observer_observation(observations, "ppu_pixel_phase_fault");
    assert_eq!(
        ppu_pixel_observation["role"],
        Value::String("expected_failure_fixture".to_string())
    );
    assert_eq!(
        ppu_pixel_observation["outcome"],
        Value::String("expected_baseline_divergence".to_string())
    );
    assert_eq!(
        ppu_pixel_observation["health"],
        Value::String("host_validation_failed".to_string())
    );
    assert_eq!(
        ppu_pixel_observation["focus_domain"],
        Value::String("ppu.pixel_phase".to_string())
    );
    assert_eq!(
        ppu_pixel_observation["next_artifact"],
        Value::String("ppu_pixel_phase_fault/comparison.json".to_string())
    );
    let ppu_attribute_observation =
        find_observer_observation(observations, "ppu_attribute_quadrant_fault");
    assert_eq!(
        ppu_attribute_observation["role"],
        Value::String("expected_failure_fixture".to_string())
    );
    assert_eq!(
        ppu_attribute_observation["outcome"],
        Value::String("expected_baseline_divergence".to_string())
    );
    assert_eq!(
        ppu_attribute_observation["health"],
        Value::String("host_validation_failed".to_string())
    );
    assert_eq!(
        ppu_attribute_observation["focus_domain"],
        Value::String("ppu.attribute_quadrant".to_string())
    );
    assert_eq!(
        ppu_attribute_observation["next_artifact"],
        Value::String("ppu_attribute_quadrant_fault/comparison.json".to_string())
    );
    ...
    assert!(observer_report.contains("| known_divergence | inspect_known_divergence | ppu_attribute_quadrant_fault | ppu_attribute_quadrant_fault/comparison.json |"));
    ...
    assert!(observer_report.contains("| known_divergence | inspect_known_divergence | cpu_branch_matrix_fault | cpu_branch_matrix_fault/comparison.json |"));
    assert!(observer_report.contains("| known_divergence | inspect_known_divergence | cpu_stack_matrix_fault | cpu_stack_matrix_fault/comparison.json |"));
    assert!(observer_report.contains("| known_divergence | inspect_known_divergence | cpu_interrupt_matrix_fault | cpu_interrupt_matrix_fault/comparison.json |"));
    assert!(observer_report.contains("| known_divergence | inspect_known_divergence | cpu_accumulator_matrix_fault | cpu_accumulator_matrix_fault/comparison.json |"));
    assert!(observer_report.contains("| known_divergence | inspect_known_divergence | cpu_compare_matrix_fault | cpu_compare_matrix_fault/comparison.json |"));
    assert!(observer_report.contains("| known_divergence | inspect_known_divergence | cpu_load_store_matrix_fault | cpu_load_store_matrix_fault/comparison.json |"));
    assert!(observer_report.contains("| known_divergence | inspect_known_divergence | cpu_alu_index_matrix_fault | cpu_alu_index_matrix_fault/comparison.json |"));
    assert!(observer_report.contains("| known_divergence | inspect_known_divergence | cpu_arithmetic_matrix_fault | cpu_arithmetic_matrix_fault/comparison.json |"));
    assert!(observer_report.contains("| known_divergence | inspect_known_divergence | cpu_status_matrix_fault | cpu_status_matrix_fault/comparison.json |"));
    ...
    assert!(observer_report.contains("| ppu_pixel_phase_fault | expected_failure_fixture | expected_baseline_divergence | host_validation_failed | ppu.pixel_phase |"));
    assert!(observer_report.contains("| ppu_attribute_quadrant_fault | expected_failure_fixture | expected_baseline_divergence | host_validation_failed | ppu.attribute_quadrant |"));
    ...
    assert!(observer_report.contains("| cpu_branch_matrix_fault | expected_failure_fixture | expected_baseline_divergence | cartridge_assertion_failed | cpu.branch.condition_matrix |"));
    assert!(observer_report.contains("| cpu_stack_matrix_fault | expected_failure_fixture | expected_baseline_divergence | cartridge_assertion_failed | cpu.stack.status_matrix |"));
    assert!(observer_report.contains("| cpu_interrupt_matrix_fault | expected_failure_fixture | expected_baseline_divergence | cartridge_assertion_failed | cpu.interrupt.brk_rti_matrix |"));
    assert!(observer_report.contains("| cpu_accumulator_matrix_fault | expected_failure_fixture | expected_baseline_divergence | cartridge_assertion_failed | cpu.accumulator.shift_rotate |"));
    assert!(observer_report.contains("| cpu_compare_matrix_fault | expected_failure_fixture | expected_baseline_divergence | cartridge_assertion_failed | cpu.compare.flags |"));
    assert!(observer_report.contains("| cpu_load_store_matrix_fault | expected_failure_fixture | expected_baseline_divergence | cartridge_assertion_failed | cpu.load_store.transfer_matrix |"));
    assert!(observer_report.contains("| cpu_alu_index_matrix_fault | expected_failure_fixture | expected_baseline_divergence | cartridge_assertion_failed | cpu.alu_index.logic_flags |"));
    assert!(observer_report.contains("| cpu_arithmetic_matrix_fault | expected_failure_fixture | expected_baseline_divergence | cartridge_assertion_failed | cpu.arithmetic.adc_sbc_flags |"));
    assert!(observer_report.contains("| cpu_status_matrix_fault | expected_failure_fixture | expected_baseline_divergence | cartridge_assertion_failed | cpu.status.bit_flags |"));
    ...
    assert!(suite_report.contains("| Baseline divergences | 37 |"));
    ...
    assert!(suite_report.contains("| known_divergence | ppu_pixel_phase_fault | scenario_diverges_from_pass_baseline | host_validation_failed | ppu.pixel_phase |"));
    assert!(suite_report.contains("| known_divergence | ppu_attribute_quadrant_fault | scenario_diverges_from_pass_baseline | host_validation_failed | ppu.attribute_quadrant |"));
    ...
    assert!(suite_report.contains("| known_divergence | cpu_branch_matrix_fault | scenario_diverges_from_pass_baseline | cartridge_assertion_failed | cpu.branch.condition_matrix |"));
    assert!(suite_report.contains("| known_divergence | cpu_stack_matrix_fault | scenario_diverges_from_pass_baseline | cartridge_assertion_failed | cpu.stack.status_matrix |"));
    assert!(suite_report.contains("| known_divergence | cpu_interrupt_matrix_fault | scenario_diverges_from_pass_baseline | cartridge_assertion_failed | cpu.interrupt.brk_rti_matrix |"));
    assert!(suite_report.contains("| known_divergence | cpu_accumulator_matrix_fault | scenario_diverges_from_pass_baseline | cartridge_assertion_failed | cpu.accumulator.shift_rotate |"));
    assert!(suite_report.contains("| known_divergence | cpu_compare_matrix_fault | scenario_diverges_from_pass_baseline | cartridge_assertion_failed | cpu.compare.flags |"));
    assert!(suite_report.contains("| known_divergence | cpu_load_store_matrix_fault | scenario_diverges_from_pass_baseline | cartridge_assertion_failed | cpu.load_store.transfer_matrix |"));
    assert!(suite_report.contains("| known_divergence | cpu_alu_index_matrix_fault | scenario_diverges_from_pass_baseline | cartridge_assertion_failed | cpu.alu_index.logic_flags |"));
    assert!(suite_report.contains("| known_divergence | cpu_arithmetic_matrix_fault | scenario_diverges_from_pass_baseline | cartridge_assertion_failed | cpu.arithmetic.adc_sbc_flags |"));
    assert!(suite_report.contains("| known_divergence | cpu_status_matrix_fault | scenario_diverges_from_pass_baseline | cartridge_assertion_failed | cpu.status.bit_flags |"));
    ...
    assert!(
        suite_report.contains("| input_mask_matrix_pass | true | true | true | healthy | 47 | - |")
    );
    assert!(suite_report
        .contains("| input_mask_all_released_pass | true | true | true | healthy | 47 | - |"));
    assert!(suite_report
        .contains("| input_mask_all_pressed_pass | true | true | true | healthy | 47 | - |"));
    assert!(suite_report
        .contains("| input_mask_joypad1_pressed_pass | true | true | true | healthy | 47 | - |"));
    assert!(suite_report
        .contains("| input_mask_joypad2_pressed_pass | true | true | true | healthy | 47 | - |"));
    assert!(suite_report
        .contains("| input_mask_sparse_bits_pass | true | true | true | healthy | 47 | - |"));
    assert!(suite_report
        .contains("| input_mask_nibble_split_pass | true | true | true | healthy | 47 | - |"));
    ...
    assert!(suite_report.contains("| cpu_branch_matrix_fault | false | false | true | cartridge_assertion_failed | 39 | cpu.branch.condition_matrix |"));
    assert!(suite_report.contains("| cpu_stack_matrix_fault | false | false | true | cartridge_assertion_failed | 40 | cpu.stack.status_matrix |"));
    assert!(suite_report.contains("| cpu_interrupt_matrix_fault | false | false | true | cartridge_assertion_failed | 41 | cpu.interrupt.brk_rti_matrix |"));
    assert!(suite_report.contains("| cpu_accumulator_matrix_fault | false | false | true | cartridge_assertion_failed | 42 | cpu.accumulator.shift_rotate |"));
    assert!(suite_report.contains("| cpu_compare_matrix_fault | false | false | true | cartridge_assertion_failed | 43 | cpu.compare.flags |"));
    assert!(suite_report.contains("| cpu_load_store_matrix_fault | false | false | true | cartridge_assertion_failed | 44 | cpu.load_store.transfer_matrix |"));
    assert!(suite_report.contains("| cpu_alu_index_matrix_fault | false | false | true | cartridge_assertion_failed | 45 | cpu.alu_index.logic_flags |"));
    assert!(suite_report.contains("| cpu_arithmetic_matrix_fault | false | false | true | cartridge_assertion_failed | 46 | cpu.arithmetic.adc_sbc_flags |"));
    assert!(suite_report.contains("| cpu_status_matrix_fault | false | false | true | cartridge_assertion_failed | 47 | cpu.status.bit_flags |"));
    ...
    assert!(suite_report.contains("| ppu_pixel_phase_fault | false | false | true | host_validation_failed | 48 | ppu.pixel_phase |"));
    assert!(suite_report.contains("| ppu_attribute_quadrant_fault | false | false | true | host_validation_failed | 49 | ppu.attribute_quadrant |"));
    ...
    assert!(suite_report.contains("| cpu_branch_matrix_fault | true | true | true | true | true |"));
    assert!(suite_report.contains("| cpu_stack_matrix_fault | true | true | true | true | true |"));
    assert!(
        suite_report.contains("| cpu_interrupt_matrix_fault | true | true | true | true | true |")
    );
    assert!(suite_report
        .contains("| cpu_accumulator_matrix_fault | true | true | true | true | true |"));
    assert!(
        suite_report.contains("| cpu_compare_matrix_fault | true | true | true | true | true |")
    );
    assert!(
        suite_report.contains("| cpu_load_store_matrix_fault | true | true | true | true | true |")
    );
    assert!(
        suite_report.contains("| cpu_alu_index_matrix_fault | true | true | true | true | true |")
    );
    assert!(
        suite_report.contains("| cpu_arithmetic_matrix_fault | true | true | true | true | true |")
    );
    assert!(suite_report.contains("| cpu_status_matrix_fault | true | true | true | true | true |"));
    ...
    assert_eq!(pass["actual_focus_test_id"], Value::from(47));
    ...
    assert_eq!(pass["contract"]["expected_focus_test_id"], Value::from(47));
    assert_eq!(pass["contract"]["actual_focus_test_id"], Value::from(47));
    ...
    assert_eq!(input_matrix["actual_focus_test_id"], Value::from(47));
    ...
    let branch = find_scenario(scenarios, "cpu_branch_matrix_fault");
    assert_eq!(
        branch["actual_health"],
        Value::String("cartridge_assertion_failed".to_string())
    );
    assert_eq!(branch["actual_focus_test_id"], Value::from(39));
    assert_eq!(
        branch["actual_focus_domain"],
        Value::String("cpu.branch.condition_matrix".to_string())
    );
    assert_eq!(branch["expectation_met"], Value::Bool(true));
    assert_eq!(branch["contract"]["all_matched"], Value::Bool(true));
    assert_eq!(
        branch["contract"]["expected_focus_domain"],
        Value::String("cpu.branch.condition_matrix".to_string())
    );
    assert!(branch["failed_probe_ids"]
        .as_array()
        .expect("CPU branch failed probes should be an array")
        .iter()
        .any(|probe| probe == &Value::String("cartridge.test.39.result".to_string())));
    assert_eq!(branch["comparison"]["passed"], Value::Bool(false));
    assert!(
        branch["comparison"]["difference_count"]
            .as_u64()
            .expect("CPU branch comparison difference_count should be numeric")
            > 0
    );
    let branch_triage = read_json(
        &suite_dir
            .join("cpu_branch_matrix_fault")
            .join("triage.json"),
    );
    assert_eq!(
        branch_triage["input"]["fault_injection"],
        Value::String("cpu_branch_condition_matrix".to_string())
    );
    assert_eq!(
        branch_triage["debug_focus"]["focus_test_name"],
        Value::String("cpu_branch_condition_matrix".to_string())
    );
    assert_eq!(
        branch_triage["failure"]["likely_domain"],
        Value::String("cpu.branch.condition_matrix".to_string())
    );
    assert!(branch_triage["probes"]["failed"]
        .as_array()
        .expect("CPU branch triage failed probes should be an array")
        .iter()
        .any(|probe| probe["id"] == Value::String("cartridge.test.39.result".to_string())));
    assert_replay_args_contains(&branch["replay_args"], "--fault-injection");
    assert_replay_args_contains(&branch["replay_args"], "cpu_branch_condition_matrix");
    assert_bundle_artifacts_with_config(
        &suite_dir.join("cpu_branch_matrix_fault"),
        true,
        false,
        "0x28",
    ...
    let stack = find_scenario(scenarios, "cpu_stack_matrix_fault");
    assert_eq!(
        stack["actual_health"],
        Value::String("cartridge_assertion_failed".to_string())
    );
    assert_eq!(stack["actual_focus_test_id"], Value::from(40));
    assert_eq!(
        stack["actual_focus_domain"],
        Value::String("cpu.stack.status_matrix".to_string())
    );
    assert_eq!(stack["expectation_met"], Value::Bool(true));
    assert_eq!(stack["contract"]["all_matched"], Value::Bool(true));
    assert_eq!(
        stack["contract"]["expected_focus_domain"],
        Value::String("cpu.stack.status_matrix".to_string())
    );
    assert!(stack["failed_probe_ids"]
        .as_array()
        .expect("CPU stack failed probes should be an array")
        .iter()
        .any(|probe| probe == &Value::String("cartridge.test.40.result".to_string())));
    assert_eq!(stack["comparison"]["passed"], Value::Bool(false));
    assert!(
        stack["comparison"]["difference_count"]
            .as_u64()
            .expect("CPU stack comparison difference_count should be numeric")
            > 0
    );
    let stack_triage = read_json(&suite_dir.join("cpu_stack_matrix_fault").join("triage.json"));
    assert_eq!(
        stack_triage["input"]["fault_injection"],
        Value::String("cpu_stack_status_matrix".to_string())
    );
    assert_eq!(
        stack_triage["debug_focus"]["focus_test_name"],
        Value::String("cpu_stack_status_matrix".to_string())
    );
    assert_eq!(
        stack_triage["failure"]["likely_domain"],
        Value::String("cpu.stack.status_matrix".to_string())
    );
    assert!(stack_triage["probes"]["failed"]
        .as_array()
        .expect("CPU stack triage failed probes should be an array")
        .iter()
        .any(|probe| probe["id"] == Value::String("cartridge.test.40.result".to_string())));
    assert_replay_args_contains(&stack["replay_args"], "--fault-injection");
    assert_replay_args_contains(&stack["replay_args"], "cpu_stack_status_matrix");
    assert_bundle_artifacts_with_config(
        &suite_dir.join("cpu_stack_matrix_fault"),
        true,
        false,
        "0x28",
    ...
    let interrupt = find_scenario(scenarios, "cpu_interrupt_matrix_fault");
    assert_eq!(
        interrupt["actual_health"],
        Value::String("cartridge_assertion_failed".to_string())
    );
    assert_eq!(interrupt["actual_focus_test_id"], Value::from(41));
    assert_eq!(
        interrupt["actual_focus_domain"],
        Value::String("cpu.interrupt.brk_rti_matrix".to_string())
    );
    assert_eq!(interrupt["expectation_met"], Value::Bool(true));
    assert_eq!(interrupt["contract"]["all_matched"], Value::Bool(true));
    assert_eq!(
        interrupt["contract"]["expected_focus_domain"],
        Value::String("cpu.interrupt.brk_rti_matrix".to_string())
    );
    assert!(interrupt["failed_probe_ids"]
        .as_array()
        .expect("CPU interrupt failed probes should be an array")
        .iter()
        .any(|probe| probe == &Value::String("cartridge.test.41.result".to_string())));
    assert_eq!(interrupt["comparison"]["passed"], Value::Bool(false));
    assert!(
        interrupt["comparison"]["difference_count"]
            .as_u64()
            .expect("CPU interrupt comparison difference_count should be numeric")
            > 0
    );
    let interrupt_triage = read_json(
        &suite_dir
            .join("cpu_interrupt_matrix_fault")
            .join("triage.json"),
    );
    assert_eq!(
        interrupt_triage["input"]["fault_injection"],
        Value::String("cpu_interrupt_matrix".to_string())
    );
    assert_eq!(
        interrupt_triage["debug_focus"]["focus_test_name"],
        Value::String("cpu_interrupt_brk_rti_matrix".to_string())
    );
    assert_eq!(
        interrupt_triage["failure"]["likely_domain"],
        Value::String("cpu.interrupt.brk_rti_matrix".to_string())
    );
    assert!(interrupt_triage["probes"]["failed"]
        .as_array()
        .expect("CPU interrupt triage failed probes should be an array")
        .iter()
        .any(|probe| probe["id"] == Value::String("cartridge.test.41.result".to_string())));
    assert_replay_args_contains(&interrupt["replay_args"], "--fault-injection");
    assert_replay_args_contains(&interrupt["replay_args"], "cpu_interrupt_matrix");
    assert_bundle_artifacts_with_config(
        &suite_dir.join("cpu_interrupt_matrix_fault"),
        true,
        false,
        "0x28",
    ...
    let accumulator = find_scenario(scenarios, "cpu_accumulator_matrix_fault");
    assert_eq!(
        accumulator["actual_health"],
        Value::String("cartridge_assertion_failed".to_string())
    );
    assert_eq!(accumulator["actual_focus_test_id"], Value::from(42));
    assert_eq!(
        accumulator["actual_focus_domain"],
        Value::String("cpu.accumulator.shift_rotate".to_string())
    );
    assert_eq!(accumulator["expectation_met"], Value::Bool(true));
    assert_eq!(accumulator["contract"]["all_matched"], Value::Bool(true));
    assert_eq!(
        accumulator["contract"]["expected_focus_domain"],
        Value::String("cpu.accumulator.shift_rotate".to_string())
    );
    assert!(accumulator["failed_probe_ids"]
        .as_array()
        .expect("CPU accumulator failed probes should be an array")
        .iter()
        .any(|probe| probe == &Value::String("cartridge.test.42.result".to_string())));
    assert_eq!(accumulator["comparison"]["passed"], Value::Bool(false));
    assert!(
        accumulator["comparison"]["difference_count"]
            .as_u64()
            .expect("CPU accumulator comparison difference_count should be numeric")
            > 0
    );
    let accumulator_triage = read_json(
        &suite_dir
            .join("cpu_accumulator_matrix_fault")
            .join("triage.json"),
    );
    assert_eq!(
        accumulator_triage["input"]["fault_injection"],
        Value::String("cpu_accumulator_matrix".to_string())
    );
    assert_eq!(
        accumulator_triage["debug_focus"]["focus_test_name"],
        Value::String("cpu_accumulator_shift_rotate_matrix".to_string())
    );
    assert_eq!(
        accumulator_triage["failure"]["likely_domain"],
        Value::String("cpu.accumulator.shift_rotate".to_string())
    );
    assert!(accumulator_triage["probes"]["failed"]
        .as_array()
        .expect("CPU accumulator triage failed probes should be an array")
        .iter()
        .any(|probe| probe["id"] == Value::String("cartridge.test.42.result".to_string())));
    assert_replay_args_contains(&accumulator["replay_args"], "--fault-injection");
    assert_replay_args_contains(&accumulator["replay_args"], "cpu_accumulator_matrix");
    assert_bundle_artifacts_with_config(
        &suite_dir.join("cpu_accumulator_matrix_fault"),
        true,
        false,
        "0x28",
    ...
    let compare = find_scenario(scenarios, "cpu_compare_matrix_fault");
    assert_eq!(
        compare["actual_health"],
        Value::String("cartridge_assertion_failed".to_string())
    );
    assert_eq!(compare["actual_focus_test_id"], Value::from(43));
    assert_eq!(
        compare["actual_focus_domain"],
        Value::String("cpu.compare.flags".to_string())
    );
    assert_eq!(compare["expectation_met"], Value::Bool(true));
    assert_eq!(compare["contract"]["all_matched"], Value::Bool(true));
    assert_eq!(
        compare["contract"]["expected_focus_domain"],
        Value::String("cpu.compare.flags".to_string())
    );
    assert!(compare["failed_probe_ids"]
        .as_array()
        .expect("CPU compare failed probes should be an array")
        .iter()
        .any(|probe| probe == &Value::String("cartridge.test.43.result".to_string())));
    assert_eq!(compare["comparison"]["passed"], Value::Bool(false));
    assert!(
        compare["comparison"]["difference_count"]
            .as_u64()
            .expect("CPU compare comparison difference_count should be numeric")
            > 0
    );
    let compare_triage = read_json(
        &suite_dir
            .join("cpu_compare_matrix_fault")
            .join("triage.json"),
    );
    assert_eq!(
        compare_triage["input"]["fault_injection"],
        Value::String("cpu_compare_matrix".to_string())
    );
    assert_eq!(
        compare_triage["debug_focus"]["focus_test_name"],
        Value::String("cpu_compare_register_matrix".to_string())
    );
    assert_eq!(
        compare_triage["failure"]["likely_domain"],
        Value::String("cpu.compare.flags".to_string())
    );
    assert!(compare_triage["probes"]["failed"]
        .as_array()
        .expect("CPU compare triage failed probes should be an array")
        .iter()
        .any(|probe| probe["id"] == Value::String("cartridge.test.43.result".to_string())));
    assert_replay_args_contains(&compare["replay_args"], "--fault-injection");
    assert_replay_args_contains(&compare["replay_args"], "cpu_compare_matrix");
    assert_bundle_artifacts_with_config(
        &suite_dir.join("cpu_compare_matrix_fault"),
        true,
        false,
        "0x28",
    ...
    let load_store = find_scenario(scenarios, "cpu_load_store_matrix_fault");
    assert_eq!(
        load_store["actual_health"],
        Value::String("cartridge_assertion_failed".to_string())
    );
    assert_eq!(load_store["actual_focus_test_id"], Value::from(44));
    assert_eq!(
        load_store["actual_focus_domain"],
        Value::String("cpu.load_store.transfer_matrix".to_string())
    );
    assert_eq!(load_store["expectation_met"], Value::Bool(true));
    assert_eq!(load_store["contract"]["all_matched"], Value::Bool(true));
    assert_eq!(
        load_store["contract"]["expected_focus_domain"],
        Value::String("cpu.load_store.transfer_matrix".to_string())
    );
    assert!(load_store["failed_probe_ids"]
        .as_array()
        .expect("CPU load/store failed probes should be an array")
        .iter()
        .any(|probe| probe == &Value::String("cartridge.test.44.result".to_string())));
    assert_eq!(load_store["comparison"]["passed"], Value::Bool(false));
    assert!(
        load_store["comparison"]["difference_count"]
            .as_u64()
            .expect("CPU load/store comparison difference_count should be numeric")
            > 0
    );
    let load_store_triage = read_json(
        &suite_dir
            .join("cpu_load_store_matrix_fault")
            .join("triage.json"),
    );
    assert_eq!(
        load_store_triage["input"]["fault_injection"],
        Value::String("cpu_load_store_matrix".to_string())
    );
    assert_eq!(
        load_store_triage["debug_focus"]["focus_test_name"],
        Value::String("cpu_load_store_transfer_matrix".to_string())
    );
    assert_eq!(
        load_store_triage["failure"]["likely_domain"],
        Value::String("cpu.load_store.transfer_matrix".to_string())
    );
    assert!(load_store_triage["probes"]["failed"]
        .as_array()
        .expect("CPU load/store triage failed probes should be an array")
        .iter()
        .any(|probe| probe["id"] == Value::String("cartridge.test.44.result".to_string())));
    assert_replay_args_contains(&load_store["replay_args"], "--fault-injection");
    assert_replay_args_contains(&load_store["replay_args"], "cpu_load_store_matrix");
    assert_bundle_artifacts_with_config(
        &suite_dir.join("cpu_load_store_matrix_fault"),
        true,
        false,
        "0x28",
    ...
    let alu_index = find_scenario(scenarios, "cpu_alu_index_matrix_fault");
    assert_eq!(
        alu_index["actual_health"],
        Value::String("cartridge_assertion_failed".to_string())
    );
    assert_eq!(alu_index["actual_focus_test_id"], Value::from(45));
    assert_eq!(
        alu_index["actual_focus_domain"],
        Value::String("cpu.alu_index.logic_flags".to_string())
    );
    assert_eq!(alu_index["expectation_met"], Value::Bool(true));
    assert_eq!(alu_index["contract"]["all_matched"], Value::Bool(true));
    assert_eq!(
        alu_index["contract"]["expected_focus_domain"],
        Value::String("cpu.alu_index.logic_flags".to_string())
    );
    assert!(alu_index["failed_probe_ids"]
        .as_array()
        .expect("CPU ALU/index failed probes should be an array")
        .iter()
        .any(|probe| probe == &Value::String("cartridge.test.45.result".to_string())));
    assert_eq!(alu_index["comparison"]["passed"], Value::Bool(false));
    assert!(
        alu_index["comparison"]["difference_count"]
            .as_u64()
            .expect("CPU ALU/index comparison difference_count should be numeric")
            > 0
    );
    let alu_index_triage = read_json(
        &suite_dir
            .join("cpu_alu_index_matrix_fault")
            .join("triage.json"),
    );
    assert_eq!(
        alu_index_triage["input"]["fault_injection"],
        Value::String("cpu_alu_index_matrix".to_string())
    );
    assert_eq!(
        alu_index_triage["debug_focus"]["focus_test_name"],
        Value::String("cpu_alu_index_matrix".to_string())
    );
    assert_eq!(
        alu_index_triage["failure"]["likely_domain"],
        Value::String("cpu.alu_index.logic_flags".to_string())
    );
    assert!(alu_index_triage["probes"]["failed"]
        .as_array()
        .expect("CPU ALU/index triage failed probes should be an array")
        .iter()
        .any(|probe| probe["id"] == Value::String("cartridge.test.45.result".to_string())));
    assert_replay_args_contains(&alu_index["replay_args"], "--fault-injection");
    assert_replay_args_contains(&alu_index["replay_args"], "cpu_alu_index_matrix");
    assert_bundle_artifacts_with_config(
        &suite_dir.join("cpu_alu_index_matrix_fault"),
        true,
        false,
        "0x28",
    ...
    let arithmetic = find_scenario(scenarios, "cpu_arithmetic_matrix_fault");
    assert_eq!(
        arithmetic["actual_health"],
        Value::String("cartridge_assertion_failed".to_string())
    );
    assert_eq!(arithmetic["actual_focus_test_id"], Value::from(46));
    assert_eq!(
        arithmetic["actual_focus_domain"],
        Value::String("cpu.arithmetic.adc_sbc_flags".to_string())
    );
    assert_eq!(arithmetic["expectation_met"], Value::Bool(true));
    assert_eq!(arithmetic["contract"]["all_matched"], Value::Bool(true));
    assert_eq!(
        arithmetic["contract"]["expected_focus_domain"],
        Value::String("cpu.arithmetic.adc_sbc_flags".to_string())
    );
    assert!(arithmetic["failed_probe_ids"]
        .as_array()
        .expect("CPU arithmetic failed probes should be an array")
        .iter()
        .any(|probe| probe == &Value::String("cartridge.test.46.result".to_string())));
    assert_eq!(arithmetic["comparison"]["passed"], Value::Bool(false));
    assert!(
        arithmetic["comparison"]["difference_count"]
            .as_u64()
            .expect("CPU arithmetic comparison difference_count should be numeric")
            > 0
    );
    let arithmetic_triage = read_json(
        &suite_dir
            .join("cpu_arithmetic_matrix_fault")
            .join("triage.json"),
    );
    assert_eq!(
        arithmetic_triage["input"]["fault_injection"],
        Value::String("cpu_arithmetic_flag_matrix".to_string())
    );
    assert_eq!(
        arithmetic_triage["debug_focus"]["focus_test_name"],
        Value::String("cpu_arithmetic_flag_matrix".to_string())
    );
    assert_eq!(
        arithmetic_triage["failure"]["likely_domain"],
        Value::String("cpu.arithmetic.adc_sbc_flags".to_string())
    );
    assert!(arithmetic_triage["probes"]["failed"]
        .as_array()
        .expect("CPU arithmetic triage failed probes should be an array")
        .iter()
        .any(|probe| probe["id"] == Value::String("cartridge.test.46.result".to_string())));
    assert_replay_args_contains(&arithmetic["replay_args"], "--fault-injection");
    assert_replay_args_contains(&arithmetic["replay_args"], "cpu_arithmetic_flag_matrix");
    assert_bundle_artifacts_with_config(
        &suite_dir.join("cpu_arithmetic_matrix_fault"),
        true,
        false,
        "0x28",
    ...
    let status = find_scenario(scenarios, "cpu_status_matrix_fault");
    assert_eq!(
        status["actual_health"],
        Value::String("cartridge_assertion_failed".to_string())
    );
    assert_eq!(status["actual_focus_test_id"], Value::from(47));
    assert_eq!(
        status["actual_focus_domain"],
        Value::String("cpu.status.bit_flags".to_string())
    );
    assert_eq!(status["expectation_met"], Value::Bool(true));
    assert_eq!(status["contract"]["all_matched"], Value::Bool(true));
    assert_eq!(
        status["contract"]["expected_focus_domain"],
        Value::String("cpu.status.bit_flags".to_string())
    );
    assert!(status["failed_probe_ids"]
        .as_array()
        .expect("CPU status/BIT failed probes should be an array")
        .iter()
        .any(|probe| probe == &Value::String("cartridge.test.47.result".to_string())));
    assert_eq!(status["comparison"]["passed"], Value::Bool(false));
    assert!(
        status["comparison"]["difference_count"]
            .as_u64()
            .expect("CPU status/BIT comparison difference_count should be numeric")
            > 0
    );
    let status_triage = read_json(
        &suite_dir
            .join("cpu_status_matrix_fault")
            .join("triage.json"),
    );
    assert_eq!(
        status_triage["input"]["fault_injection"],
        Value::String("cpu_status_bit_matrix".to_string())
    );
    assert_eq!(
        status_triage["debug_focus"]["focus_test_name"],
        Value::String("cpu_status_bit_matrix".to_string())
    );
    assert_eq!(
        status_triage["failure"]["likely_domain"],
        Value::String("cpu.status.bit_flags".to_string())
    );
    assert!(status_triage["probes"]["failed"]
        .as_array()
        .expect("CPU status/BIT triage failed probes should be an array")
        .iter()
        .any(|probe| probe["id"] == Value::String("cartridge.test.47.result".to_string())));
    assert_replay_args_contains(&status["replay_args"], "--fault-injection");
    assert_replay_args_contains(&status["replay_args"], "cpu_status_bit_matrix");
    assert_bundle_artifacts_with_config(
        &suite_dir.join("cpu_status_matrix_fault"),
        true,
        false,
        "0x28",
    ...
    let ppu_pixel_phase = find_scenario(scenarios, "ppu_pixel_phase_fault");
    assert_eq!(
        ppu_pixel_phase["actual_health"],
        Value::String("host_validation_failed".to_string())
    );
    assert_eq!(ppu_pixel_phase["actual_focus_test_id"], Value::from(48));
    assert_eq!(
        ppu_pixel_phase["actual_focus_domain"],
        Value::String("ppu.pixel_phase".to_string())
    );
    assert_eq!(ppu_pixel_phase["expectation_met"], Value::Bool(true));
    assert_eq!(
        ppu_pixel_phase["contract"]["all_matched"],
        Value::Bool(true)
    );
    assert_eq!(
        ppu_pixel_phase["contract"]["expected_focus_domain"],
        Value::String("ppu.pixel_phase".to_string())
    );
    assert!(ppu_pixel_phase["failed_probe_ids"]
        .as_array()
        .expect("PPU pixel-phase failed probes should be an array")
        .iter()
        .any(|probe| probe == &Value::String("ppu.pixel_phase.signature".to_string())));
    assert_eq!(ppu_pixel_phase["comparison"]["passed"], Value::Bool(false));
    assert!(
        ppu_pixel_phase["comparison"]["difference_count"]
            .as_u64()
            .expect("PPU pixel-phase comparison difference_count should be numeric")
            > 0
    );
    let ppu_pixel_phase_triage =
        read_json(&suite_dir.join("ppu_pixel_phase_fault").join("triage.json"));
    assert_eq!(
        ppu_pixel_phase_triage["input"]["fault_injection"],
        Value::String("ppu_pixel_phase".to_string())
    );
    assert_eq!(
        ppu_pixel_phase_triage["debug_focus"]["failure_kind"],
        Value::String("host_validation".to_string())
    );
    assert_eq!(
        ppu_pixel_phase_triage["failure"]["likely_domain"],
        Value::String("ppu.pixel_phase".to_string())
    );
    assert!(ppu_pixel_phase_triage["probes"]["failed"]
        .as_array()
        .expect("PPU pixel-phase triage failed probes should be an array")
        .iter()
        .any(|probe| probe["id"] == Value::String("ppu.pixel_phase.signature".to_string())));
    assert_bundle_artifacts_with_config(
        &suite_dir.join("ppu_pixel_phase_fault"),
        true,
        false,
        "0x28",
    ...
    let ppu_attribute_quadrant = find_scenario(scenarios, "ppu_attribute_quadrant_fault");
    assert_eq!(
        ppu_attribute_quadrant["actual_health"],
        Value::String("host_validation_failed".to_string())
    );
    assert_eq!(
        ppu_attribute_quadrant["actual_focus_test_id"],
        Value::from(49)
    );
    assert_eq!(
        ppu_attribute_quadrant["actual_focus_domain"],
        Value::String("ppu.attribute_quadrant".to_string())
    );
    assert_eq!(ppu_attribute_quadrant["expectation_met"], Value::Bool(true));
    assert_eq!(
        ppu_attribute_quadrant["contract"]["all_matched"],
        Value::Bool(true)
    );
    assert_eq!(
        ppu_attribute_quadrant["contract"]["expected_focus_domain"],
        Value::String("ppu.attribute_quadrant".to_string())
    );
    assert!(ppu_attribute_quadrant["failed_probe_ids"]
        .as_array()
        .expect("PPU attribute-quadrant failed probes should be an array")
        .iter()
        .any(|probe| probe == &Value::String("ppu.attribute_quadrant.signature".to_string())));
    assert_eq!(
        ppu_attribute_quadrant["comparison"]["passed"],
        Value::Bool(false)
    );
    assert!(
        ppu_attribute_quadrant["comparison"]["difference_count"]
            .as_u64()
            .expect("PPU attribute-quadrant comparison difference_count should be numeric")
            > 0
    );
    let ppu_attribute_quadrant_triage = read_json(
        &suite_dir
            .join("ppu_attribute_quadrant_fault")
            .join("triage.json"),
    );
    assert_eq!(
        ppu_attribute_quadrant_triage["input"]["fault_injection"],
        Value::String("ppu_attribute_quadrant".to_string())
    );
    assert_eq!(
        ppu_attribute_quadrant_triage["debug_focus"]["failure_kind"],
        Value::String("host_validation".to_string())
    );
    assert_eq!(
        ppu_attribute_quadrant_triage["failure"]["likely_domain"],
        Value::String("ppu.attribute_quadrant".to_string())
    );
    assert!(ppu_attribute_quadrant_triage["probes"]["failed"]
        .as_array()
        .expect("PPU attribute-quadrant triage failed probes should be an array")
        .iter()
        .any(|probe| probe["id"] == Value::String("ppu.attribute_quadrant.signature".to_string())));
    assert_bundle_artifacts_with_config(
        &suite_dir.join("ppu_attribute_quadrant_fault"),
        true,
        false,
        "0x28",
...
fn diagnostic_cli_writes_standalone_triage_json() {
    ...
    assert_eq!(triage["telemetry_schema_version"], Value::from(76));
    ...
    assert_eq!(triage["coverage"]["passed_tests"], Value::from(41));
