# Diagnostic CPU Load/Store LDA Indirect-Indexed

## Goal

Extend the generated `cpu_load_store_transfer_matrix` diagnostic cartridge with
an explicit `LDA (zp),Y` load case so AI/debug telemetry can distinguish
indirect-indexed load-addressing regressions inside the broader CPU
load/store/transfer family.

## Acceptance

- The generated diagnostic cartridge still runs headlessly to pass.
- `cpu_load_store_matrix` records a fifth load subcase and expected load mask
  `0x1F`.
